...
/// soft_op with both identities: a+b, a-b, a*b, a/b, a%b, a, b
/// Used in array accumulators where body update can be either
/// `acc = acc OP rhs` (fold) or `acc = rhs` (assignment for max/min).
const N_OPS_ASSIGN: usize = 7;

fn soft_op_assign(a: f32, b: f32, weights: &[f32]) -> f32 {
    let safe_b = if b.abs() < 1e-6 { 1.0 } else { b };
    let results = [
        a + b,
        a - b,
