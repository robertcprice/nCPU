...
fn softmax_temp(logits: &[f32], temp: f32) -> Vec<f32> {
    ...
    let mut exps: Vec<f32> = logits.iter().map(|&x| ((x * inv_temp) - scaled_max).exp()).collect();
    let sum: f32 = exps.iter().sum();
    let inv_sum = 1.0 / sum;
    for e in &mut exps { *e *= inv_sum; }
    exps
}
...
impl Adam {
    ...
    fn step(&mut self, params: &mut [f32], grads: &[f32]) {
        self.t += 1;
        let lr_t = self.lr
            * (1.0 - self.b2.powi(self.t as i32)).sqrt()
...
impl SoftExprProgram {
    ...
    fn forward(&self, inputs: &[f32], temp: f32) -> f32 {
        ...
        let s2 = soft_read(&ext, &softmax_temp(&self.params[off + ne..off + 2 * ne], temp));
    ...
    fn discretize_and_emit(&self, fn_name: &str, param_names: &[&str]) -> String {
        ...
        if pre_on
            && pre_s1_idx < src_names.len() - 1
            && pre_s2_idx < src_names.len() - 1
        {
            let _ = writeln!(
                body,
                "    v0: i64 = {} {} {};",
...
impl SoftTwoPrecompExprProgram {
    fn ns(&self) -> usize { self.n_args + N_CONSTS }
    fn ne1(&self) -> usize { self.n_args + N_CONSTS + 1 }
    fn ne2(&self) -> usize { self.n_args + N_CONSTS + 2 }
    ...
    fn new(n_args: usize) -> Self {
        ...
        if n_args > 0 { p[1] = 2.0; }
        let ps2 = 1 + ns;
        if n_args > 1 { p[ps2 + 1] = 2.0; } else { p[ps2 + n_args.saturating_sub(1).min(ns-1)] = 2.0; }
        ...
        if n_args > 0 { p[roff] = 2.0; }
        let rs2 = roff + ne2;
        if n_args > 1 { p[rs2 + 1] = 2.0; } else { p[rs2 + n_args] = 2.0; }
        p[roff + 2*ne2] = 2.0; // ret_op = +

        // consts: [0, 1, -1, 2, -2, 10]
        let coff = roff + 2*ne2 + N_OPS;
        p[coff] = 0.0; p[coff+1] = 1.0; p[coff+2] = -1.0;
        p[coff+3] = 2.0; p[coff+4] = -2.0; p[coff+5] = 10.0;
        Self { n_args, params: p }
    }

    fn const_offset(&self) -> usize {
        let ns = self.ns(); let ne1 = self.ne1(); let ne2 = self.ne2();
    ...
    fn forward(&self, inputs: &[f32], temp: f32) -> f32 {
        ...
        let mut storage = vec![0.0f32; ns];
        for (i, &v) in inputs.iter().take(self.n_args).enumerate() { storage[i] = v; }
        for (i, &c) in consts.iter().enumerate() { storage[self.n_args + i] = c; }

        // Pre1
        let pre1_en = sigmoid(self.params[0]);
        let p1s1 = soft_read(&storage, &softmax_temp(&self.params[1..1+ns], temp));
        let p1s2 = soft_read(&storage, &softmax_temp(&self.params[1+ns..1+2*ns], temp));
        let p1op = softmax_temp(&self.params[1+2*ns..1+2*ns+N_OPS], temp);
        let v0 = soft_op(p1s1, p1s2, &p1op) * pre1_en;

        let mut ext1 = storage.clone(); ext1.push(v0);

        // Pre2
        let p2 = 1 + 2*ns + N_OPS;
        let pre2_en = sigmoid(self.params[p2]);
        let p2s1 = soft_read(&ext1, &softmax_temp(&self.params[p2+1..p2+1+ne1], temp));
        let p2s2 = soft_read(&ext1, &softmax_temp(&self.params[p2+1+ne1..p2+1+2*ne1], temp));
        let p2op = softmax_temp(&self.params[p2+1+2*ne1..p2+1+2*ne1+N_OPS], temp);
        let v1 = soft_op(p2s1, p2s2, &p2op) * pre2_en;

        let mut ext2 = ext1; ext2.push(v1);

        // Return
        let roff = p2 + 1 + 2*ne1 + N_OPS;
        let rs1 = soft_read(&ext2, &softmax_temp(&self.params[roff..roff+ne2], temp));
        let rs2 = soft_read(&ext2, &softmax_temp(&self.params[roff+ne2..roff+2*ne2], temp));
        let rop = softmax_temp(&self.params[roff+2*ne2..roff+2*ne2+N_OPS], temp);
        soft_op(rs1, rs2, &rop)
    }

    fn loss(&self, examples: &[(Vec<f32>, f32)], temp: f32) -> f32 {
        let preds: Vec<f32> = examples.iter().map(|(inp, _)| self.forward(inp, temp)).collect();
        let targets: Vec<f32> = examples.iter().map(|(_, t)| *t).collect();
        mse(&preds, &targets)
    }
    ...
    fn discretize_and_emit(&self, fn_name: &str, param_names: &[&str]) -> String {
        ...
        let mut src_names: Vec<String> = param_names.iter().map(|s| s.to_string()).collect();
        for c in &consts { src_names.push(format!("{}", c.round() as i64)); }
        src_names.push("v0".to_string());
        let src_names_ext: Vec<String> = {
            let mut v = src_names.clone(); v.push("v1".to_string()); v
        };
        ...
        let params_sig = param_names.iter().map(|n| format!("{n}: i64")).collect::<Vec<_>>().join(", ");
        let mut body = String::new();

        if pre1_on && p1s1i < ns && p1s2i < ns {
            let n1 = &src_names[p1s1i]; let n2 = &src_names[p1s2i];
            let _ = writeln!(body, "    v0: i64 = {n1} {} {n2};", ops[p1opi.min(N_OPS-1)]);
        }
        if pre2_on && p2s1i < ne1 && p2s2i < ne1 {
            let n1 = &src_names[p2s1i.min(src_names.len()-1)];
            let n2 = &src_names[p2s2i.min(src_names.len()-1)];
            let _ = writeln!(body, "    v1: i64 = {n1} {} {n2};", ops[p2opi.min(N_OPS-1)]);
        }
...
impl SoftBranchProgram {
    ...
    fn new(n_args: usize) -> Self {
        ...
        for b in 1..N_BRANCHES {
            ...
            for k in 0..N_CMPS - 1 { p[bo + k] = -8.0; }
    ...
    fn forward(&self, inputs: &[f32], temp: f32) -> f32 {
        ...
        for b in 0..N_BRANCHES {
            ...
            let rhs = soft_read(
                &ext,
                &softmax_temp(
                    &self.params[bo + N_CMPS + ne..bo + N_CMPS + 2 * ne],
                    temp,
                ),
            );
...
impl SoftLoopProgram {
    ...
    fn discretize_and_emit(&self, fn_name: &str, param_names: &[&str]) -> String {
        ...
        let mut bound_src_names: Vec<String> =
...
impl SoftTwoAccLoop {
    fn na(&self) -> usize { 2 + self.n_args + N_CONSTS }
    fn nb(&self) -> usize { self.n_args + N_CONSTS }
    ...
    fn forward(&self, inputs: &[f32], temp: f32) -> f32 {
        let na = self.na(); let nb = self.nb();
        let ret_off = 2 + nb + 1 + 4*na + 12;
        let coff = ret_off + na;
        let consts: Vec<f32> = (0..N_CONSTS).map(|i| self.params[coff + i]).collect();

        let mut base = vec![0.0f32; self.n_args + N_CONSTS];
        for (i, &v) in inputs.iter().take(self.n_args).enumerate() { base[i] = v; }
        for (i, &c) in consts.iter().enumerate() { base[self.n_args + i] = c; }
    ...
    fn discretize_and_emit(&self, fn_name: &str, param_names: &[&str]) -> String {
        ...
        for n in param_names { snames.push(n.to_string()); }
        for c in &consts { snames.push(format!("{}", c.round() as i64)); }

        let mut bnames: Vec<String> = param_names.iter().map(|s| s.to_string()).collect();
        for c in &consts { bnames.push(format!("{}", c.round() as i64)); }

        let ops = ["+", "-", "*", "/", "%", ""];
        let bname = bnames.get(bound_idx).cloned().unwrap_or_else(|| "n".to_string());
        let bexpr = if bound_off == 0 { bname.clone() }
            else if bound_off > 0 { format!("{} + {}", bname, bound_off) }
            else { format!("{} - {}", bname, -bound_off) };
        ...
        let a_expr = if aopi >= N_OPS { as1n.to_string() }
            else { format!("{as1n} {} {as2n}", ops[aopi]) };
        // x1_expr: new value for x1 (uses OLD x0, x1 — emitted BEFORE x0 is updated)
        let b_expr = if bopi >= N_OPS { bs1n.to_string() }
            else { format!("{bs1n} {} {bs2n}", ops[bopi]) };
...
impl SoftPredicateLoop {
    ...
    fn new(n_args: usize) -> Self {
        ...
        if n_args > 1 { p[nb + 1] = 2.0; } else { p[nb + n_args] = 2.0; }
    ...
    fn discretize_and_emit(&self, fn_name: &str, param_names: &[&str]) -> String {
        ...
        let init_names: Vec<String> = param_names.iter().map(|s| s.to_string())
        ...
        for n in param_names { lnames.push(n.to_string()); }
        for c in &consts { lnames.push(format!("{}", c.round() as i64)); }
        ...
        let a_expr = if aop_i >= N_OPS { as1_s.to_string() }
            else { format!("{as1_s} {} {as2_s}", ops[aop_i]) };
        let b_expr = if bop_i >= N_OPS { bs1_s.to_string() }
            else { format!("{bs1_s} {} {bs2_s}", ops[bop_i]) };
...
impl SoftPredicateLoopRetCmp {
    fn base_n_params(n_args: usize) -> usize { SoftPredicateLoop::n_params(n_args) }
    fn ret_cmp_off(n_args: usize) -> usize { Self::base_n_params(n_args) }
    fn ret_rhs_off(n_args: usize) -> usize { Self::base_n_params(n_args) + N_CMPS }
    fn n_params(n_args: usize) -> usize {
        let na = 2 + n_args + N_CONSTS;
    ...
    fn forward(&self, inputs: &[f32], temp: f32) -> f32 {
        ...
        let inner = SoftPredicateLoop { n_args: self.n_args, params: self.params[..Self::base_n_params(self.n_args)].to_vec() };
        ...
        for &v in inputs.iter().take(self.n_args) { rhs_pool.push(v); }
        for &c in &consts { rhs_pool.push(c); }
    ...
    fn discretize_and_emit(&self, fn_name: &str, param_names: &[&str]) -> String {
        ...
        for n in param_names { rhs_names.push(n.to_string()); }
        for c in &consts { rhs_names.push(format!("{}", c.round() as i64)); }
...
impl SoftCondAccumLoop {
    ...
    fn offsets(ns: usize) -> (usize, usize, usize, usize, usize, usize, usize, usize, usize) {
        let pre_op_off = 3 + ns;
        ...
        (pre_op_off, pre_s1_off, pre_s2_off, cmp_op_off, cmp_s1_off, cmp_s2_off, loop_op_off, loop_rhs_off, consts_off)
    }

    fn new(n_args: usize) -> Self {
        let ns = n_args + N_CONSTS;
        let (pre_op_off, pre_s1_off, pre_s2_off, cmp_op_off, cmp_s1_off, cmp_s2_off, loop_op_off, loop_rhs_off, consts_off) = Self::offsets(ns);
        let n_params = consts_off + N_CONSTS;
        let mut p = vec![0.0f32; n_params];
        p[0] = 0.0; // init = 0
        p[1] = 1.0; // start = 1
        if n_args > 0 { p[2] = 2.0; } // bound = arg0
        p[2 + ns] = 0.0; // bound_offset = 0
        p[pre_op_off + 4] = 1.0; // pre_op = % (default for divisibility check)
        if n_args > 0 { p[pre_s1_off] = 1.0; } // pre_s1 = arg0
        ...
        // consts: [0, 1, -1, 2, -2, 10]
        p[consts_off] = 0.0; p[consts_off + 1] = 1.0; p[consts_off + 2] = -1.0;
        p[consts_off + 3] = 2.0; p[consts_off + 4] = -2.0; p[consts_off + 5] = 10.0;
        Self { n_args, params: p }
    }
    ...
    fn discretize_and_emit(&self, fn_name: &str, param_names: &[&str]) -> String {
        ...
        for c in &consts { bound_names.push(format!("{}", c.round() as i64)); }
        ...
        let bound_name = bound_names.get(bound_idx).cloned().unwrap_or_else(|| "n".to_string());
        let bound_expr = if bound_offset == 0 {
            bound_name
        } else if bound_offset > 0 {
            format!("{} + {}", bound_name, bound_offset)
        ...
        let ps1 = pre_names.get(pre_s1_idx).cloned().unwrap_or_else(|| "n".to_string());
        let ps2 = pre_names.get(pre_s2_idx).cloned().unwrap_or_else(|| "i".to_string());
        let cs1 = cmp_names_arr.get(cmp_s1_idx).cloned().unwrap_or_else(|| "v0".to_string());
        let cs2 = cmp_names_arr.get(cmp_s2_idx).cloned().unwrap_or_else(|| "0".to_string());
        let lrhs = rhs_names.get(loop_rhs_idx).cloned().unwrap_or_else(|| "1".to_string());
...
impl SoftCondAccumCmpReturnLoop {
    fn ns(n_args: usize) -> usize { n_args + N_CONSTS }
    ...
    fn ret_cmp_off(ns: usize) -> usize { Self::base_n_params(ns) }
    fn ret_c_off(ns: usize) -> usize { Self::base_n_params(ns) + N_CMPS }
    fn n_params(n_args: usize) -> usize { Self::base_n_params(Self::ns(n_args)) + N_CMPS + 1 }
    ...
    fn forward(&self, inputs: &[f32], temp: f32) -> f32 {
        ...
        let inner = SoftCondAccumLoop { n_args: self.n_args, params: self.params[..Self::base_n_params(ns)].to_vec() };
...
impl SoftCondMutateLoop {
    fn nb(n_args: usize) -> usize { n_args + N_CONSTS }
    fn na(n_args: usize) -> usize { 1 + n_args + N_CONSTS }

    fn offsets(n_args: usize) -> (usize,usize,usize,usize, usize,usize,usize, usize,usize, usize,usize,usize, usize,usize,usize,usize,usize, usize) {
        ...
        let co           = fs3_off + ng;
        (init_off, cond_cmp_off, cond_lhs_off, cond_rhs_off,
         pre_op_off, pre_s1_off, pre_s2_off,
         gate_cmp_off, gate_rhs_off,
         true_op_off, true_s1_off, true_s2_off,
         fop1_off, fs1_off, fs2_off, fop2_off, fs3_off, co)
    }

    fn n_params(n_args: usize) -> usize {
        let (_, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _fs3_off, co) =
            Self::offsets(n_args);
    ...
    fn new(n_args: usize) -> Self {
        let n = Self::n_params(n_args);
        let (_, cond_cmp_off, cond_lhs_off, cond_rhs_off,
             pre_op_off, pre_s1_off, pre_s2_off,
             gate_cmp_off, gate_rhs_off,
             true_op_off, true_s1_off, true_s2_off,
             fop1_off, fs1_off, fs2_off, fop2_off, fs3_off, co) = Self::offsets(n_args);
        let na = Self::na(n_args);
        let ng = na + 1;
        let mut p = vec![0.0f32; n];
        // Consts
        p[co]=0.0; p[co+1]=1.0; p[co+2]=-1.0; p[co+3]=2.0; p[co+4]=-2.0; p[co+5]=10.0;
        // init_x = arg0
        p[0] = 2.0;
    ...
    fn forward(&self, inputs: &[f32], temp: f32) -> f32 {
        ...
        let ng = na + 1;
        let (init_off, cond_cmp_off, cond_lhs_off, cond_rhs_off,
             pre_op_off, pre_s1_off, pre_s2_off,
             gate_cmp_off, gate_rhs_off,
             true_op_off, true_s1_off, true_s2_off,
             fop1_off, fs1_off, fs2_off, fop2_off, fs3_off, co) = Self::offsets(self.n_args);
        ...
        for _ in 0..MAX_LOOP_ITER {
            ...
            let cond_lhs = soft_read(&pool, &softmax_temp(&self.params[cond_lhs_off..cond_lhs_off+na], temp));
            let cond_rhs = soft_read(&pool, &softmax_temp(&self.params[cond_rhs_off..cond_rhs_off+na], temp));
            ...
            let pre_s1 = soft_read(&pool, &softmax_temp(&self.params[pre_s1_off..pre_s1_off+na], temp));
            let pre_s2 = soft_read(&pool, &softmax_temp(&self.params[pre_s2_off..pre_s2_off+na], temp));
            ...
            let gate_rhs = soft_read(&gate_pool, &softmax_temp(&self.params[gate_rhs_off..gate_rhs_off+ng], temp));
            ...
            let true_s1 = soft_read(&pool, &softmax_temp(&self.params[true_s1_off..true_s1_off+na], temp));
            let true_s2 = soft_read(&pool, &softmax_temp(&self.params[true_s2_off..true_s2_off+na], temp));
            ...
            let fs1 = soft_read(&pool, &softmax_temp(&self.params[fs1_off..fs1_off+na], temp));
            let fs2 = soft_read(&pool, &softmax_temp(&self.params[fs2_off..fs2_off+na], temp));
            ...
            let fs3 = soft_read(&false_pool2, &softmax_temp(&self.params[fs3_off..fs3_off+ng], temp));
    ...
    fn discretize_and_emit(&self, fn_name: &str, param_names: &[&str]) -> String {
        ...
        for n in param_names { pool_names.push(n.to_string()); }
        for c in &consts { pool_names.push(format!("{}", c.round() as i64)); }

        let mut gate_names = vec!["pre".to_string(), "x".to_string()];
        for n in param_names { gate_names.push(n.to_string()); }
        for c in &consts { gate_names.push(format!("{}", c.round() as i64)); }

        let mut fs3_names = vec!["v_tmp".to_string(), "x".to_string()];
        for n in param_names { fs3_names.push(n.to_string()); }
        for c in &consts { fs3_names.push(format!("{}", c.round() as i64)); }

        let mut init_names: Vec<String> = param_names.iter().map(|s| s.to_string()).collect();
        for c in &consts { init_names.push(format!("{}", c.round() as i64)); }

        macro_rules! pick { ($off:expr, $pool:expr, $n:expr) => {
            $pool.get(argmax(&self.params[$off..$off+$n]).min($n-1)).map(|s| s.as_str()).unwrap_or("0")
        }}
        macro_rules! pick_op { ($off:expr) => {
            argmax(&self.params[$off..$off+CML_OP]).min(CML_OP-1)
        }}
        macro_rules! op_expr { ($a:expr, $b:expr, $oi:expr) => {
            if $oi >= N_OPS { $a.to_string() } else { format!("{} {} {}", $a, ops[$oi], $b) }
        }}
...
const CDLOOP_N_PARAMS: usize = 1 + N_CONSTS + N_CONSTS + CDLOOP_POOL + N_CMPS + CDLOOP_POOL + CDLOOP_POOL + N_OPS + N_CONSTS + 1; // 61
...
impl SoftCondDigitLoop {
    fn new() -> Self {
        ...
        p[co] = 0.0; p[co+1] = 1.0; p[co+2] = -1.0;
        p[co+3] = 2.0; p[co+4] = -2.0; p[co+5] = 10.0;
    ...
    fn offsets() -> (usize, usize, usize, usize, usize, usize, usize, usize) {
        ...
        // consts_off = 54
        (init_off, base_off, gate_pre_off, gate_lhs_off, gate_cmp_off, gate_rhs_off, acc_rhs_off, loop_op_off)
    }

    fn consts_off() -> usize { CDLOOP_N_PARAMS - N_CONSTS - 1 }
    fn zero_return_off() -> usize { CDLOOP_N_PARAMS - 1 }

    fn forward(&self, inputs: &[f32], temp: f32) -> f32 {
        let p = &self.params;
        let (_, base_off, gate_pre_off, gate_lhs_off, gate_cmp_off, gate_rhs_off, acc_rhs_off, loop_op_off) = Self::offsets();
        let co = Self::consts_off();
        ...
        for _ in 0..MAX_DIGIT_ITER {
            ...
            let pool = [d, pre, d - acc, acc, consts[0], consts[1], consts[2], consts[3], consts[4], consts[5]];
    ...
    fn discretize_and_emit(&self, fn_name: &str, param_names: &[&str]) -> String {
        ...
        let safe_gpc = if gate_pre_val == 0 { 2i64 } else { gate_pre_val.abs() };
...
impl SoftChainedBranch {
    fn pool1_size(&self) -> usize { self.n_args + N_CONSTS }
    fn pool2_size(&self) -> usize { self.n_args + N_CONSTS + 1 } // + v0
    ...
    fn offsets(n_args: usize) -> (usize, usize, usize, usize, usize, usize, usize, usize, usize, usize, usize) {
        let p1 = n_args + N_CONSTS;
        let p2 = p1 + 1;
        let b1_cmp_off = 0;
        ...
        (b1_cmp_off, b1_lhs_off, b1_rhs_off, b1_true_off, b1_false_off,
         b2_cmp_off, b2_lhs_off, b2_rhs_off, b2_true_off, b2_false_off, consts_off)
    }
    ...
    fn forward(&self, inputs: &[f32], temp: f32) -> f32 {
        ...
        let (b1_cmp_off, b1_lhs_off, b1_rhs_off, b1_true_off, b1_false_off,
             b2_cmp_off, b2_lhs_off, b2_rhs_off, b2_true_off, b2_false_off, co) = Self::offsets(na);
        let consts = [p[co], p[co+1], p[co+2], p[co+3], p[co+4], p[co+5]];

        // Pool1: [args..., consts...]
        let mut pool1 = Vec::with_capacity(p1);
        for i in 0..na { pool1.push(inputs.get(i).copied().unwrap_or(0.0)); }
        for c in &consts { pool1.push(*c); }
        ...
        let b1_true = soft_read(&pool1, &softmax_temp(&p[b1_true_off..b1_true_off + p1], temp));
        let b1_false = soft_read(&pool1, &softmax_temp(&p[b1_false_off..b1_false_off + p1], temp));
        ...
        let b2_true = soft_read(&pool2, &softmax_temp(&p[b2_true_off..b2_true_off + p2], temp));
        let b2_false = soft_read(&pool2, &softmax_temp(&p[b2_false_off..b2_false_off + p2], temp));
    ...
    fn discretize_and_emit(&self, fn_name: &str, param_names: &[&str]) -> String {
        ...
        for c in &consts { pool1_names.push(format!("{}", c.round() as i64)); }
    ...
    G: Fn(&[f32], &str, &[&str]) -> String,
{
    ...
    for step in 0..n_steps {
        ...
        let should_check = loss < 1.0
            ...
            || (step % 50 == 49); // also check every 50 steps
        if should_check {
...
#[inline]
fn univ_pool(n_args: usize) -> usize { n_args + N_CONSTS + N_UNIV_SLOTS }
#[inline]
fn univ_lip(n_args: usize) -> usize { n_args + N_CONSTS + N_INIT_SLOTS } // loop init pool
#[inline]
fn univ_sps(pool: usize) -> usize { (N_OPS + 1) + 5 * pool + N_CMPS } // slot params size
...
impl SoftUniversalProgram {
    fn ps(&self) -> usize { univ_pool(self.n_args) }
    fn lip(&self) -> usize { univ_lip(self.n_args) }
    fn sps(&self) -> usize { univ_sps(self.ps()) }

    fn slot_off(&self, slot: usize) -> usize { slot * self.sps() }

    fn loop_init_off(&self, ls: usize) -> usize {
        N_UNIV_SLOTS * self.sps() + ls * self.lip()
    }
    fn loop_cond_off(&self) -> usize { self.loop_init_off(N_LOOP_SLOTS) }
    fn return_off(&self) -> usize { self.loop_cond_off() + N_CMPS + 2 * self.ps() }
    fn consts_off(&self) -> usize { self.return_off() + self.ps() }

    fn n_params_for(n_args: usize) -> usize {
        let pool = univ_pool(n_args);
        let lip  = univ_lip(n_args);
        N_UNIV_SLOTS * univ_sps(pool) + N_LOOP_SLOTS * lip
            + N_CMPS + 2 * pool + pool + N_CONSTS
    }

    fn new(n_args: usize) -> Self {
        let mut s = Self { n_args, params: vec![0f32; Self::n_params_for(n_args)] };
        let co = s.consts_off();
    ...
    fn exec_slot(&self, slot: usize, r: &[f32], temp: f32) -> f32 {
        ...
        let gl_w   = softmax_temp(&self.params[cmp_base + N_CMPS..cmp_base + N_CMPS + pool], temp);
        let gr_w   = softmax_temp(&self.params[cmp_base + N_CMPS + pool..cmp_base + N_CMPS + 2*pool], temp);
        let gate_lhs = soft_read(r, &gl_w);
        let gate_rhs = soft_read(r, &gr_w);
        let gate   = soft_cmp(gate_lhs, gate_rhs, &cmp_w, temp);
        // else branch
        let el_w   = softmax_temp(&self.params[cmp_base + N_CMPS + 2*pool..cmp_base + N_CMPS + 3*pool], temp);
    ...
    fn forward(&self, inputs: &[f32], temp: f32) -> f32 {
        ...
        for i in 0..n.min(inputs.len()) { reg[i] = inputs[i]; }
        let co = self.consts_off();
        for i in 0..N_CONSTS { reg[n + i] = self.params[co + i]; }
        ...
        for _iter in 0..MAX_LOOP_ITER {
            ...
            let rhs_w = softmax_temp(&self.params[lco + N_CMPS + pool..lco + N_CMPS + 2*pool], temp);
    ...
    fn discretize_and_emit(&self, fn_name: &str, param_names: &[&str]) -> String {
        ...
        for i in 0..n { pn.push(param_names.get(i).copied().unwrap_or("x").to_string()); }
        for v in &consts { pn.push(format!("{v}")); }
        for i in 0..N_INIT_SLOTS { pn.push(format!("v{i}")); }
        for i in 0..N_LOOP_SLOTS { pn.push(format!("s{i}")); }
        for i in 0..N_POST_SLOTS { pn.push(format!("p{i}")); }

        // Loop-init pool names (only pre-loop registers)
        let mut lpn: Vec<String> = Vec::with_capacity(lip);
        for i in 0..n { lpn.push(param_names.get(i).copied().unwrap_or("x").to_string()); }
        for v in &consts { lpn.push(format!("{v}")); }
        for i in 0..N_INIT_SLOTS { lpn.push(format!("v{i}")); }
        ...
        let slot_line = |slot: usize, dest_name: &str, decl: bool| -> String {
            ...
            let s1 = &pn[s1_i];
            let s2 = &pn[s2_i];
            let then_expr = if op_i >= N_OPS { s1.clone() } // identity
                else { format!("{s1} {} {s2}", op_names[op_i]) };
            ...
            let prefix = if decl { format!("{dest_name}: i64") } else { dest_name.to_string() };
    ...
    fn disc_exec_slot(&self, slot: usize, reg: &[i64]) -> Option<i64> {
        ...
        let then_val = match op_i {
            ...
            1 => s1.checked_sub(s2)?,
            2 => s1.checked_mul(s2)?,
            3 => { if s2 == 0 { return None; } s1 / s2 },
            4 => { if s2 == 0 { return None; } s1 % s2 },
            _ => s1,  // identity
        };
        let gl = *reg.get(gl_i)?;
        let gr = *reg.get(gr_i)?;
        let gate = match cmp_i % N_CMPS {
            0 => gl < gr, 1 => gl <= gr, 2 => gl == gr,
            3 => gl >= gr, 4 => gl > gr, _ => gl != gr,
        };
        if gate { Some(then_val) } else { reg.get(el_i).copied() }
    }
    ...
    pub fn discrete_eval(&self, int_inputs: &[i64]) -> Option<i64> {
        ...
        for i in 0..n.min(int_inputs.len()) { reg[i] = int_inputs[i]; }
        for i in 0..N_CONSTS { reg[n + i] = self.params[co + i].round() as i64; }
...
impl UniversalProgramDescription {
    ...
    pub fn lip_names(&self) -> Vec<String> {
        ...
        let mut pn = Vec::new();
        for i in 0..self.n_args { pn.push(default_args.get(i).copied().unwrap_or("x").to_string()); }
        for v in &self.consts { pn.push(format!("{}", v.round() as i64)); }
        for i in 0..N_INIT_SLOTS { pn.push(format!("v{i}")); }
        pn
    }
    ...
    pub fn explain(&self) -> String {
        ...
        let phase_for = |s: usize| if s < N_INIT_SLOTS { "init" }
                        else if s < N_INIT_SLOTS + N_LOOP_SLOTS { "loop" } else { "post" };
        let dest_for  = |s: usize| if s < N_INIT_SLOTS { format!("v{s}") }
                        else if s < N_INIT_SLOTS + N_LOOP_SLOTS { format!("s{}", s - N_INIT_SLOTS) }
                        else { format!("p{}", s - N_INIT_SLOTS - N_LOOP_SLOTS) };
        ...
        for (i, sd) in self.slots.iter().enumerate() {
            out.push_str(&format!(
                "  [{:4}] {} = if {} {} {} {{ {} {} {} }} else {{ {} }}\n",
                phase_for(i), dest_for(i),
                pn_get(sd.gate_lhs), cmp_sym.get(sd.gate_cmp).copied().unwrap_or("?"), pn_get(sd.gate_rhs),
                pn_get(sd.s1), op_sym.get(sd.op).copied().unwrap_or("?"), pn_get(sd.s2),
                pn_get(sd.else_val),
            ));
        }
        for (ls, &src) in self.loop_init.iter().enumerate() {
            out.push_str(&format!("  [loop_init] s{ls} = {}\n", lip_pn.get(src).map(|s| s.as_str()).unwrap_or("?")));
        }
...
impl SoftUniversalProgram {
    ...
    pub fn params_to_description(&self) -> UniversalProgramDescription {
        ...
        let slots: Vec<SlotDesc> = (0..N_UNIV_SLOTS).map(|slot| {
            let off  = self.slot_off(slot);
            ...
            SlotDesc { op, s1, s2, gate_cmp, gate_lhs, gate_rhs, else_val }
        }).collect();

        let loop_init: Vec<usize> = (0..N_LOOP_SLOTS).map(|ls| {
            let io = self.loop_init_off(ls);
        ...
        for i in 0..N_CONSTS { consts[i] = self.params[co + i]; }

        UniversalProgramDescription {
            n_args: self.n_args, slots, loop_init,
            cond_cmp, cond_lhs, cond_rhs, ret_src, consts,
        }
    }
    ...
    pub fn description_to_params(desc: &UniversalProgramDescription) -> Self {
        ...
        for p in prog.params.iter_mut() { *p = -4.0; }
        ...
        for i in 0..N_CONSTS { prog.params[co + i] = desc.consts[i]; }
    ...
    pub fn pool_names(n_args: usize, consts: &[f32]) -> Vec<String> {
        let default_args = ["a", "b", "c", "d", "e", "f"];
        let mut pn = Vec::new();
        for i in 0..n_args { pn.push(default_args.get(i).copied().unwrap_or("x").to_string()); }
        for v in consts.iter().take(N_CONSTS) { pn.push(format!("{}", v.round() as i64)); }
        for i in 0..N_INIT_SLOTS { pn.push(format!("v{i}")); }
        for i in 0..N_LOOP_SLOTS { pn.push(format!("s{i}")); }
        for i in 0..N_POST_SLOTS { pn.push(format!("p{i}")); }
        pn
...
pub fn rand_description(n_args: usize, seed: u64) -> UniversalProgramDescription {
    ...
    let slots: Vec<SlotDesc> = (0..N_UNIV_SLOTS).map(|_| SlotDesc {
        op:       next(N_OPS + 1),  // 0-5
        s1:       next(pool),
        s2:       next(pool),
    ...
    let const_pool: [f32; 12] = [0.0, 1.0, -1.0, 2.0, -2.0, 3.0, 10.0, 100.0, -3.0, 4.0, 5.0, -10.0];
    let mut consts = [0f32; N_CONSTS];
    for c in &mut consts { *c = const_pool[next(const_pool.len())]; }

    UniversalProgramDescription { n_args, slots, loop_init, cond_cmp, cond_lhs, cond_rhs, ret_src, consts }
}
...
pub fn synthesize_universal_and_collect(
    ...
    let examples: Vec<(Vec<f32>, f32)> = problem.examples.iter().map(|ex| {
        let inputs: Vec<f32> = ex.inputs.iter().filter_map(|v| {
            if let Value::Int(i) = v { Some(*i as f32) } else { None }
        }).collect();
    ...
    for restart in 0..N_RESTARTS {
        ...
        if let Some((result, params)) = train_program_collect(
            prog.params.clone(),
            move |p, t| SoftUniversalProgram { n_args, params: p.to_vec() }.loss(&ex, t),
            |p, fn_n, pn| SoftUniversalProgram { n_args, params: p.to_vec() }.discretize_and_emit(fn_n, pn),
            problem, &param_names, fn_name, univ_steps,
        ) {
    ...
    G: Fn(&[f32], &str, &[&str]) -> String,
{
    ...
    for step in 0..n_steps {
        ...
        let temp = (2.0f32 * (1.0 - step as f32 / n_steps as f32)).max(0.1);
        let loss = loss_fn(&params, temp);
        if loss < best_loss { best_loss = loss; best_params = params.clone(); }
        ...
        if should_check {
            ...
            if best_loss < loss {
                if let Some(result) = try_emit_verify(&best_params, &emit_fn, problem, fn_name, param_names) {
...
pub fn synthesize_universal_warm_start(
    ...
    let fn_name = problem.function_name();
    let n_args  = problem.examples.first()?.inputs.len();
    if n_args != desc.n_args { return None; }
    ...
    let (warm_result, steps) = train_program_count_steps(
        ...
        move |p, t| SoftUniversalProgram { n_args, params: p.to_vec() }.loss(&ex, t),
        |p, fn_n, pn| SoftUniversalProgram { n_args, params: p.to_vec() }.discretize_and_emit(fn_n, pn),
        problem, &param_names, fn_name, warm_steps,
    );
    ...
    // ── Cold fallback ─────────────────────────────────────────────────────────
    if cold_restarts == 0 { return None; }
    ...
    for univ_restart in 0..cold_restarts {
        ...
        let (res, _s) = train_program_count_steps(
            prog.params.clone(),
            move |p, t| SoftUniversalProgram { n_args, params: p.to_vec() }.loss(&ex2, t),
            |p, fn_n, pn| SoftUniversalProgram { n_args, params: p.to_vec() }.discretize_and_emit(fn_n, pn),
            problem, &param_names, fn_name, COLD_STEPS,
        );
...
fn synthesize_scalar_inner(problem: &Problem, use_templates: bool) -> Option<SolveResult> {
    ...
    if !problem.examples.iter().all(|ex| {
        ex.inputs
            .iter()
    ...
    for restart in 0..N_RESTARTS {
        ...
        if n_args >= 2 && restart == 0 {
            ...
            if n_args > 1 { prog.params[boff + N_CMPS + ne + 1] = 4.0; } // rhs = arg1
        ...
        if n_args == 1 && restart == 0 {
            ...
            prog.params[boff] = -4.0; prog.params[boff + 1] = 4.0; // cmp = <
            prog.params[boff + N_CMPS] = 4.0;                        // lhs = arg0
            ...
            prog.params[b1] = 4.0; prog.params[b1 + 1] = -4.0;      // cmp = >
        ...
        if n_args == 1 && restart == 0 {
            ...
            for k in 0..N_CMPS { prog.params[b1 + k] = -4.0; }
        ...
        if n_args == 2 && restart == 0 {
            ...
            for k in 0..N_CMPS { prog.params[boff2 + k] = -4.0; }
            prog.params[boff2 + 4] = 4.0;            // cmp = ==
            for k in 0..ne2 { prog.params[boff2 + N_CMPS + k] = -4.0; }
            prog.params[boff2 + N_CMPS + 1] = 4.0;   // lhs = arg1=b
            prog.params[boff2 + N_CMPS + ne2 + 2] = 4.0; // rhs = const[0]=0
            for k in 0..ne2 { prog.params[boff2 + N_CMPS + 2*ne2 + k] = -4.0; }
        ...
        if n_args == 1 && restart == 0 {
            ...
            for k in 0..N_CMPS { prog.params[b1o + k] = -4.0; }
        ...
        // 2b. SoftChainedBranch — two sequential ternary branches (min3, max3, clamp)
        {
            ...
            if restart > 0 {
                ...
                for (idx, p) in prog.params.iter_mut().enumerate() {
                    *p += (pseudo_rand(restart as u64 * 13000 + idx as u64) - 0.5) * noise_scale * 0.5;
                }
                prog.params[co]=0.0; prog.params[co+1]=1.0; prog.params[co+2]=-1.0;
                prog.params[co+3]=2.0; prog.params[co+4]=-2.0; prog.params[co+5]=10.0;
            }
            let ex = examples.clone();
            let result = train_program(
                prog.params.clone(),
        ...
        if n_args == 3 {
            ...
            let (b1_cmp_off, b1_lhs_off, b1_rhs_off, b1_true_off, b1_false_off,
                 b2_cmp_off, b2_lhs_off, b2_rhs_off, b2_true_off, b2_false_off, co) = SoftChainedBranch::offsets(n_args);
            for p in prog.params.iter_mut() { *p = -4.0; }
            prog.params[co]=0.0; prog.params[co+1]=1.0; prog.params[co+2]=-1.0;
            prog.params[co+3]=2.0; prog.params[co+4]=-2.0; prog.params[co+5]=10.0;
            ...
            if restart > 0 {
                let hot: std::collections::HashSet<usize> = [
                    b1_cmp_off+1, b1_lhs_off, b1_rhs_off+1, b1_true_off, b1_false_off+1,
                    b2_cmp_off+1, b2_lhs_off+p2-1, b2_rhs_off+2, b2_true_off+p2-1, b2_false_off+2,
                    co, co+1, co+2, co+3, co+4, co+5,
                ].iter().copied().collect();
                for (idx, p) in prog.params.iter_mut().enumerate() {
                    if !hot.contains(&idx) {
                        *p += (pseudo_rand(restart as u64 * 17000 + idx as u64) - 0.5) * noise_scale * 0.3;
                    }
                }
            }
            let ex = examples.clone();
            let result = train_program(
                ...
                move |p, t| SoftChainedBranch { n_args, params: p.to_vec() }.loss(&ex, t),
                |p, fn_n, pn| SoftChainedBranch { n_args, params: p.to_vec() }.discretize_and_emit(fn_n, pn),
                problem, &param_names, fn_name, N_STEPS * 2,
            );
            if result.is_some() { return result; }
        }
        ...
        // 3. SoftLoopProgram (all n_args, not just 1)
        {
            ...
            let result = train_program(
                prog.params.clone(),
                move |p, t| SoftLoopProgram { n_args, params: p.to_vec() }.loss(&ex, t),
                |p, fn_n, pn| SoftLoopProgram { n_args, params: p.to_vec() }.discretize_and_emit(fn_n, pn),
                problem, &param_names, fn_name, N_STEPS,
            );
            if result.is_some() { return result; }
        }
        ...
        if n_args == 1 && restart == 0 {
            ...
            let opoff = 2 + nb + 1;
            for k in 0..N_OPS { prog.params[opoff + k] = -4.0; }
            prog.params[opoff + 2] = 4.0;  // op = * (index 2)
            // rhs = i (index 0)
            let rhsoff = opoff + N_OPS;
            for k in 0..nr { prog.params[rhsoff + k] = -2.0; }
        ...
        if n_args == 1 && restart == 0 {
            ...
            let opoff = 2 + nb + 1;
            for k in 0..N_OPS { prog.params[opoff + k] = -2.0; }
        ...
        if n_args == 2 && restart == 0 {
            ...
            for k in 0..nb { prog.params[2 + k] = -2.0; }
        ...
        if n_args == 1 && restart == 0 {
            ...
            for k in 0..nb { prog.params[2 + k] = -4.0; }
            prog.params[2] = 4.0;  // bound = arg0 (n)
            prog.params[2 + nb] = 0.0;  // bound_offset = 0
            for k in 0..N_OPS { prog.params[opoff_h + k] = -4.0; }
            prog.params[opoff_h] = 4.0;  // op = +
            for k in 0..nr_new { prog.params[rhsoff_h + k] = -4.0; }
        ...
        if n_args == 1 && restart == 0 {
            ...
            let result = train_program(
                prog.params.clone(),
                move |p, t| SoftDigitLoopProgram { params: p.to_vec() }.loss(&ex, t),
                |p, fn_n, pn| SoftDigitLoopProgram { params: p.to_vec() }.discretize_and_emit(fn_n, pn),
                problem, &param_names, fn_name, N_STEPS,
            );
            if result.is_some() { return result; }
        }
        ...
        if n_args == 1 {
            {
                ...
                let result = train_program(
                    prog.params.clone(),
                    move |p, t| SoftTwoAccLoop { n_args, params: p.to_vec() }.loss(&ex, t),
                    |p, fn_n, pn| SoftTwoAccLoop { n_args, params: p.to_vec() }.discretize_and_emit(fn_n, pn),
                    problem, &param_names, fn_name, N_STEPS,
                );
                if result.is_some() { return result; }
            }
            ...
            if restart == 0 {
                let mut prog = SoftTwoAccLoop::new(n_args);
                prog.params[0] = 2.0; prog.params[1] = 1.0; // a=2, b=1
                let na = prog.na(); let nb_bound = prog.nb();
                ...
                let bs1_off = aop_off + 6;
                let bs2_off = bs1_off + na;
                prog.params[bs1_off] = 4.0; prog.params[bs2_off + 1] = 4.0;
                let bop_off = bs2_off + na;
        ...
        if n_args == 1 {
            {
                ...
                let result = train_program(
                    prog.params.clone(),
                    move |p, t| SoftCondAccumLoop { n_args, params: p.to_vec() }.loss(&ex, t),
                    |p, fn_n, pn| SoftCondAccumLoop { n_args, params: p.to_vec() }.discretize_and_emit(fn_n, pn),
                    problem, &param_names, fn_name, N_STEPS,
                );
                if result.is_some() { return result; }
            }
            ...
            if restart == 0 {
                let mut prog = SoftCondAccumLoop::new(n_args);
                let ns = n_args + N_CONSTS; // 7
                let (pre_op_off, pre_s1_off, pre_s2_off, cmp_op_off, cmp_s1_off, cmp_s2_off, loop_op_off, loop_rhs_off, co) = SoftCondAccumLoop::offsets(ns);
                for p in prog.params.iter_mut() { *p = -4.0; }
                prog.params[0] = 0.0;           // init = 0
                prog.params[1] = 1.0;           // start = 1
        ...
        if n_args == 1 {
            {
                ...
                if restart > 0 {
                    ...
                    let (_, _, _, _, _, _, _, _, co) = SoftCondAccumLoop::offsets(ns);
                    for (idx, p) in prog.params.iter_mut().enumerate() {
                        *p += (pseudo_rand(restart as u64 * 19000 + idx as u64) - 0.5) * noise_scale * 0.5;
                    }
                    prog.params[co]=0.0; prog.params[co+1]=1.0; prog.params[co+2]=-1.0;
                    prog.params[co+3]=2.0; prog.params[co+4]=-2.0; prog.params[co+5]=10.0;
                }
                ...
                let result = train_program(
                    prog.params.clone(),
                    move |p, t| SoftCondAccumCmpReturnLoop { n_args, params: p.to_vec() }.loss(&ex, t),
                    |p, fn_n, pn| SoftCondAccumCmpReturnLoop { n_args, params: p.to_vec() }.discretize_and_emit(fn_n, pn),
                    problem, &param_names, fn_name, N_STEPS,
                );
                if result.is_some() { return result; }
            }
        ...
        if n_args == 1 {
            // Generic noisy init
            {
                ...
                if restart > 0 {
                    ...
                    for (idx, p) in prog.params.iter_mut().enumerate() {
                        *p += (pseudo_rand(restart as u64 * 11000 + idx as u64) - 0.5) * noise_scale * 0.5;
                    }
                    prog.params[co]=0.0; prog.params[co+1]=1.0; prog.params[co+2]=-1.0;
                    prog.params[co+3]=2.0; prog.params[co+4]=-2.0; prog.params[co+5]=10.0;
                }
                ...
                let result = train_program(
                    ...
                    |p, fn_n, pn| SoftCondDigitLoop { params: p.to_vec() }.discretize_and_emit(fn_n, pn),
                    problem, &param_names, fn_name, N_STEPS,
                );
                if result.is_some() { return result; }
            }
            ...
            if restart == 0 {
                let mut prog = SoftCondDigitLoop::new();
                let (_, base_off, gate_pre_off, gate_lhs_off, gate_cmp_off, gate_rhs_off, acc_rhs_off, loop_op_off) = SoftCondDigitLoop::offsets();
                let co = SoftCondDigitLoop::consts_off();
                for p in prog.params.iter_mut() { *p = -4.0; }
                prog.params[co]=0.0; prog.params[co+1]=1.0; prog.params[co+2]=-1.0;
                prog.params[co+3]=2.0; prog.params[co+4]=-2.0; prog.params[co+5]=10.0;
        ...
        // 5. SoftPredicateLoop — predicate-gated while loop (GCD, leading_digit, next_power_of_2, collatz)
        {
            ...
            if restart > 0 {
                for (idx, p) in prog.params.iter_mut().enumerate() {
                    *p += (pseudo_rand(restart as u64 * 9000 + idx as u64) - 0.5) * noise_scale * 0.5;
                }
            }
            let ex = examples.clone();
            let result = train_program(
                prog.params.clone(),
                move |p, t| SoftPredicateLoop { n_args, params: p.to_vec() }.loss(&ex, t),
                |p, fn_n, pn| SoftPredicateLoop { n_args, params: p.to_vec() }.discretize_and_emit(fn_n, pn),
                problem, &param_names, fn_name, N_STEPS,
            );
            if result.is_some() { return result; }
        }
        ...
        if n_args == 2 && restart == 0 {
            let mut prog = SoftPredicateLoop::new(n_args);
            {
                let nb = prog.nb(); let na = prog.na();
                let c0 = 2*nb; let c1 = c0+N_CMPS; let c2 = c1+na;
                let a1 = c2+na; let a2 = a1+na; let ao = a2+na;
                let b1 = ao+6;  let b2 = b1+na; let bo = b2+na;
                let ro = bo+6;  let co = ro+na;
                for p in prog.params.iter_mut() { *p = -1.0; }
                prog.params[0] = 4.0; prog.params[nb+1] = 4.0;  // init0=arg0, init1=arg1
        ...
        if n_args == 1 && restart == 0 {
            ...
            let nb = base_prog.nb(); let na = base_prog.na();
            let c0 = 2*nb; let c1 = c0+N_CMPS; let c2 = c1+na;
            let a1 = c2+na; let a2 = a1+na; let ao = a2+na;
            let b1 = ao+6;  let b2 = b1+na; let bo = b2+na;
            let ro = bo+6;  let co = ro+na;
            ...
            // Pattern A: leading_digit — while x0>=10 { x0=x0/10 }, return x0
            {
                ...
                let result = train_program(
                    p,
                    move |pp, t| SoftPredicateLoop { n_args, params: pp.to_vec() }.loss(&ex, t),
                    |pp, fn_n, pn| SoftPredicateLoop { n_args, params: pp.to_vec() }.discretize_and_emit(fn_n, pn),
                    problem, &param_names, fn_name, N_STEPS,
                );
                if result.is_some() { return result; }
            }
        ...
        if n_args == 1 {
        {
            ...
            if restart > 0 {
                ...
                for (idx, p) in prog.params.iter_mut().enumerate() {
                    *p += (pseudo_rand(restart as u64 * 23000 + idx as u64) - 0.5) * noise_scale * 0.5;
                }
                let coff = base_n - N_CONSTS;
                prog.params[coff]=0.0; prog.params[coff+1]=1.0; prog.params[coff+2]=-1.0;
                prog.params[coff+3]=2.0; prog.params[coff+4]=-2.0; prog.params[coff+5]=10.0;
            }
            let ex = examples.clone();
            let result = train_program(
                prog.params.clone(),
                move |p, t| SoftPredicateLoopRetCmp { n_args, params: p.to_vec() }.loss(&ex, t),
                |p, fn_n, pn| SoftPredicateLoopRetCmp { n_args, params: p.to_vec() }.discretize_and_emit(fn_n, pn),
                problem, &param_names, fn_name, N_STEPS,
            );
            if result.is_some() { return result; }
        }
        ...
        if restart == 0 {
            ...
            let mut prog = SoftPredicateLoopRetCmp::new(n_args);
            {
                ...
                let nb = inner.nb(); let na = inner.na();
                let c0 = 2*nb; let c1 = c0+N_CMPS; let c2 = c1+na;
                let a1 = c2+na; let a2 = a1+na; let ao = a2+na;
                let b1 = ao+6;  let b2 = b1+na; let bo = b2+na;
                let ro = bo+6;  let co = ro+na;
        ...
        if n_args == 1 {
        {
            ...
            if restart > 0 {
                let (_, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, co) = SoftCondMutateLoop::offsets(n_args);
                for (idx, p) in prog.params.iter_mut().enumerate() {
                    *p += (pseudo_rand(restart as u64 * 29000 + idx as u64) - 0.5) * noise_scale * 0.5;
                }
                prog.params[co]=0.0; prog.params[co+1]=1.0; prog.params[co+2]=-1.0;
                prog.params[co+3]=2.0; prog.params[co+4]=-2.0; prog.params[co+5]=10.0;
            }
            ...
            let result = train_program(
                prog.params.clone(),
                move |p, t| SoftCondMutateLoop { n_args, params: p.to_vec() }.loss(&ex, t),
                |p, fn_n, pn| SoftCondMutateLoop { n_args, params: p.to_vec() }.discretize_and_emit(fn_n, pn),
                problem, &param_names, fn_name, N_STEPS,
            );
            if result.is_some() { return result; }
        }
        ...
        // false_s3 pool: [v_tmp=0, x=1, n=2, c0=3, c1=4, c2=5, c3=6, c4=7, c5=8]
        {
            ...
            let (init_off, cond_cmp_off, cond_lhs_off, cond_rhs_off,
                 ...
                 gate_cmp_off, gate_rhs_off,
                 true_op_off, true_s1_off, true_s2_off,
                 fop1_off, fs1_off, fs2_off, fop2_off, fs3_off, co) = SoftCondMutateLoop::offsets(n_args);
            let na = SoftCondMutateLoop::na(n_args);
            let ng = na + 1;
            for p in prog.params.iter_mut() { *p = -4.0; }
            // consts: set c4=3 instead of default -2 so x*3 is representable
            prog.params[co]=0.0; prog.params[co+1]=1.0; prog.params[co+2]=-1.0;
            prog.params[co+3]=2.0; prog.params[co+4]=3.0; prog.params[co+5]=10.0;
            ...
            if restart > 0 {
                let hot: std::collections::HashSet<usize> = [
                    co, co+1, co+2, co+3, co+4, co+5,
                    init_off,
                    cond_cmp_off+5, cond_lhs_off, cond_rhs_off+3,
                    pre_op_off+4, pre_s1_off, pre_s2_off+5,
                    gate_cmp_off+4, gate_rhs_off+3,
                    true_op_off+3, true_s1_off, true_s2_off+5,
                    fop1_off+2, fs1_off, fs2_off+6,
                    fop2_off, fs3_off+4,
                ].iter().copied().collect();
                for (idx, p) in prog.params.iter_mut().enumerate() {
                    if !hot.contains(&idx) {
                        *p += (pseudo_rand(restart as u64 * 31000 + idx as u64) - 0.5) * noise_scale * 0.3;
                    }
                }
            }
            let ex = examples.clone();
            ...
            if result.is_some() { return result; }
            let _ = (na, ng, fs3_off, cond_cmp_off, pre_s1_off, pre_s2_off,
                     gate_rhs_off, true_s1_off, true_s2_off, fs1_off, fs2_off, fop2_off);
        }
        } // if n_args == 1 (9)
    ...
    for univ_restart in 0..3 {
        ...
        let result = train_program(
            prog.params.clone(),
            move |p, t| SoftUniversalProgram { n_args, params: p.to_vec() }.loss(&ex, t),
            |p, fn_n, pn| SoftUniversalProgram { n_args, params: p.to_vec() }
                .discretize_and_emit(fn_n, pn),
            problem, &param_names, fn_name, UNIV_STEPS,
        );
        if result.is_some() { return result; }
...
mod tests {
    ...
    fn predicate_loop_biased_params_structure() {
        // GCD: init(a,b), while x1!=0 { (x0,x1)=(x1, x0%x1) }, return x0
        {
            ...
            let ro = bo+6;  let co = ro+na;
            for p in prog.params.iter_mut() { *p = -1.0; }
            prog.params[0] = 4.0; prog.params[nb+1] = 4.0;
            prog.params[c0+5] = 4.0; prog.params[c1+1] = 4.0;
            prog.params[c2+2+n_args] = 4.0;
            prog.params[a1+1] = 4.0; prog.params[ao+5] = 4.0;
            prog.params[b1+0] = 4.0; prog.params[b2+1] = 4.0; prog.params[bo+4] = 4.0;
            ...
            assert!(code.contains("x0 % x1") || code.contains("x0%x1"), "GCD body: {code}");
        ...
        // leading_digit: while x0>=10, x0=x0/10, return x0
        {
            let n_args = 1usize;
            let base = SoftPredicateLoop::new(n_args);
            let nb = base.nb(); let na = base.na();
            let c0 = 2*nb; let c1 = c0+N_CMPS; let c2 = c1+na;
            let a1 = c2+na; let a2 = a1+na; let ao = a2+na;
            let b1 = ao+6;  let b2 = b1+na; let bo = b2+na;
            let ro = bo+6;  let co = ro+na;
            let mut p = vec![-1.0f32; base.params.len()];
            p[0] = 4.0; p[nb+1] = 4.0;  // init0=arg0, init1=const[0]=0
            p[c0+2] = 4.0; p[c1+0] = 4.0; p[c2+8] = 4.0;
            p[a1+0] = 4.0; p[a2+8] = 4.0; p[ao+3] = 4.0;
            p[b1+1] = 4.0; p[bo+5] = 4.0; p[ro+0] = 4.0;
            p[co]=0.0; p[co+1]=1.0; p[co+2]=-1.0;
            p[co+3]=2.0; p[co+4]=-2.0; p[co+5]=10.0;
            let _ = b2;
            let code = SoftPredicateLoop { n_args, params: p }.discretize_and_emit("ld", &["n"]);
            println!("leading_digit:\n{code}");
            assert!(code.contains(">= 10") || code.contains(">=10"), "ld cond: {code}");
            assert!(code.contains("x0 / 10") || code.contains("x0/10"), "ld body: {code}");
        ...
        // next_power_of_2: init x0=1, while x0<n, x0=x0*2, return x0
        {
            ...
            let ro = bo+6;  let co = ro+na;
            let mut p = vec![-1.0f32; base.params.len()];
            p[2] = 4.0; p[nb+1] = 4.0;  // init0=const[1]=1, init1=const[0]=0
            p[c0+1] = 4.0; p[c1+0] = 4.0; p[c2+2] = 4.0;
            p[a1+0] = 4.0; p[a2+6] = 4.0; p[ao+2] = 4.0;
            p[b1+1] = 4.0; p[bo+5] = 4.0; p[ro+0] = 4.0;
            p[co]=0.0; p[co+1]=1.0; p[co+2]=-1.0;
            p[co+3]=2.0; p[co+4]=-2.0; p[co+5]=10.0;
            let _ = b2;
            ...
            assert!(code.contains("x0 < n") || code.contains("x0<n"), "np2 cond: {code}");
            assert!(code.contains("x0 * 2") || code.contains("x0*2"), "np2 body: {code}");
        ...
        // digit_count: x0=n, x1=1; while x0>=10 { x0=x0/10; x1=x1+1 }; return x1
        {
            ...
            let ro = bo+6;  let co = ro+na;
            let mut p = vec![-1.0f32; base.params.len()];
            p[0] = 4.0; p[nb+2] = 4.0;  // init0=arg0, init1=const[1]=1
            p[c0+2] = 4.0; p[c1+0] = 4.0; p[c2+8] = 4.0; // cmp>=, lhs=x0, rhs=10
            p[a1+0] = 4.0; p[a2+8] = 4.0; p[ao+3] = 4.0; // a: x0/10
            p[b1+1] = 4.0; p[b2+4] = 4.0; p[bo+0] = 4.0; // b: x1+1
            p[ro+1] = 4.0;                                  // ret = x1
            p[co]=0.0; p[co+1]=1.0; p[co+2]=-1.0;
            p[co+3]=2.0; p[co+4]=-2.0; p[co+5]=10.0;
            let code = SoftPredicateLoop { n_args, params: p }.discretize_and_emit("dc", &["n"]);
            println!("digit_count:\n{code}");
            assert!(code.contains(">= 10") || code.contains(">=10"), "dc cond: {code}");
            assert!(code.contains("x0 / 10") || code.contains("x0/10"), "dc a-body: {code}");
            assert!(code.contains("x1 + 1") || code.contains("x1+1"), "dc b-body: {code}");
        ...
        // safe_div_or_neg1: if b==0 return -1 else return a/b
        {
            ...
            let doff = boff + N_BRANCHES * branch_size;
            let mut prog = SoftBranchProgram::new(n_args);
            for k in 0..N_CMPS { prog.params[boff + k] = -4.0; }
            prog.params[boff + 4] = 4.0;
            for k in 0..ne { prog.params[boff + N_CMPS + k] = -4.0; }
            prog.params[boff + N_CMPS + 1] = 4.0;
            prog.params[boff + N_CMPS + ne + 2] = 4.0;
            for k in 0..ne { prog.params[boff + N_CMPS + 2*ne + k] = -4.0; }
            prog.params[boff + N_CMPS + 2*ne + 4] = 4.0;
            prog.params[boff + N_CMPS + 4*ne + 5] = 4.0;
            prog.params[doff + 1] = -4.0; prog.params[doff] = 4.0;
            prog.params[doff + ne + 1] = 4.0;
            prog.params[doff + 2*ne + 5] = -4.0; prog.params[doff + 2*ne + 3] = 4.0;
            ...
            assert!(code.contains("a / b") || code.contains("a/b"), "sdiv default: {code}");
        }

        // digit_product: mode=product, init_acc=1
        {
            let mut prog = SoftDigitLoopProgram::new();
            prog.params[0] = -4.0; prog.params[1] = 4.0;
            prog.params[2] = -4.0; prog.params[3] = -4.0;
            prog.params[4] = 1.0;
            let code = prog.discretize_and_emit("dp", &["n"]);
            println!("digit_product:\n{code}");
            assert!(code.contains("acc * (x % 10)") || code.contains("acc*(x%10)"), "dp body: {code}");
        ...
        // SoftCondAccumLoop structural test: count_divisors biased restart
        {
            ...
            prog.params[co]=0.0; prog.params[co+1]=1.0; prog.params[co+2]=-1.0;
            prog.params[co+3]=2.0; prog.params[co+4]=-2.0; prog.params[co+5]=10.0;
            prog.params[0] = 0.0; prog.params[1] = 1.0;
            prog.params[2] = 4.0; prog.params[2 + ns] = 0.0;
            prog.params[pre_op_off + 4] = 4.0;
        ...
        // SoftCondDigitLoop structural tests
        {
            ...
            assert!(code.contains("% 2") || code.contains("%2"), "ced pre: {code}");
            assert!(code.contains("== 0"), "ced gate: {code}");
            assert!(code.contains("acc + 1") || code.contains("+ 1"), "ced body: {code}");
        }
        {
            ...
            assert!(code.contains("x % 10") || code.contains("(x % 10)"), "sod rhs: {code}");
        }
        {
            ...
            assert!(code.contains("+ 1") || code.contains("acc + 1"), "pop body: {code}");
        }
        {
            ...
            assert!(code.contains("> acc") || code.contains(">acc"), "md gate: {code}");
            assert!(code.contains("- acc") || code.contains("-acc"), "md body: {code}");
        }
        ...
        // SoftChainedBranch structural test: min3 biased restart
        {
            ...
            assert!(code.contains("result = v0"), "min3 b2 true (assignment): {code}");
            assert!(code.contains("result: i64 = c"), "min3 b2 false (init): {code}");
        }
        ...
        // SoftCondAccumCmpReturnLoop structural test: is_prime biased restart
        {
            ...
            assert!(code.contains("acc + 1") || code.contains("acc +1"), "ip body: {code}");
        ...
        // SoftPredicateLoopRetCmp structural test: triangular_check biased restart
        {
            ...
            assert!(code.contains("x1 + x0") || code.contains("x1+x0"), "tc b-body: {code}");
    ...
    fn universal_description_round_trip_test() {
        ...
        assert_eq!(p0_desc.gate_lhs, p0_desc.gate_rhs, "trivially-true gate: lhs==rhs");
        ...
        println!("Description:\n{explanation}");
        assert!(explanation.contains("+ b"), "explanation should mention + b: {explanation}");
        assert!(explanation.contains("p0"), "explanation should mention p0: {explanation}");
    ...
    fn universal_description_factorial_encode_test() {
        ...
        let default_slot = SlotDesc {
            op: 5, s1: 0, s2: 0,
            gate_cmp: 2, gate_lhs: 0, gate_rhs: 0, else_val: 0,
        };
        ...
        slots[N_INIT_SLOTS + 0] = SlotDesc {
            op: 2, s1: idx_s0, s2: idx_s1, // *
            gate_cmp: 2, gate_lhs: 0, gate_rhs: 0, else_val: idx_s0,
        };
        // Loop body slot 4 (s1): s1 = s1 - 1
        slots[N_INIT_SLOTS + 1] = SlotDesc {
            op: 1, s1: idx_s1, s2: idx_c1, // -
            gate_cmp: 2, gate_lhs: 0, gate_rhs: 0, else_val: idx_s1,
        };
        ...
        let desc = UniversalProgramDescription {
            n_args, slots, loop_init, cond_cmp, cond_lhs, cond_rhs, ret_src, consts,
        };
        ...
        let prog = SoftUniversalProgram::description_to_params(&desc);
        assert_eq!(prog.params.len(), SoftUniversalProgram::n_params_for(n_args));
        let code = prog.discretize_and_emit("factorial", &["n"]);
        println!("Factorial emit:\n{code}");
        assert!(code.contains("fn factorial(n: i64) -> i64"), "header: {code}");
    ...
    fn discrete_eval_correctness_test() {
        ...
        let default_slot = SlotDesc { op: 5, s1: 0, s2: 0, gate_cmp: 2, gate_lhs: 0, gate_rhs: 0, else_val: 0 };
        let mut slots: Vec<SlotDesc> = (0..N_UNIV_SLOTS).map(|_| default_slot.clone()).collect();
        slots[N_INIT_SLOTS + 0] = SlotDesc { op: 2, s1: idx_s0, s2: idx_s1, gate_cmp: 2, gate_lhs: 0, gate_rhs: 0, else_val: idx_s0 };
        slots[N_INIT_SLOTS + 1] = SlotDesc { op: 1, s1: idx_s1, s2: idx_c1, gate_cmp: 2, gate_lhs: 0, gate_rhs: 0, else_val: idx_s1 };
        let mut loop_init = vec![1usize; N_LOOP_SLOTS];
        loop_init[0] = 2; // s0 = 1
        loop_init[1] = 0; // s1 = n
        let desc = UniversalProgramDescription {
            n_args, slots, loop_init, cond_cmp: 5, cond_lhs: idx_s1, cond_rhs: idx_c1,
            ret_src: idx_s0, consts,
        };
        let prog = SoftUniversalProgram::description_to_params(&desc);
        ...
        for p in prog2.params.iter_mut() { *p = -4.0; }
        ...
        let default_sl = SlotDesc { op: 5, s1: 0, s2: 0, gate_cmp: 2, gate_lhs: 0, gate_rhs: 0, else_val: 0 };
        ...
        slots3[0] = SlotDesc { op: 0, s1: 0, s2: idx_c_2, gate_cmp: 2, gate_lhs: 0, gate_rhs: 0, else_val: 0 };
        ...
        let desc3 = UniversalProgramDescription {
            n_args: n_args_1, slots: slots3, loop_init: loop_init3,
            cond_cmp: 2, cond_lhs: 0, cond_rhs: 0, // loop cond: 0==0, always… but trivially
            ret_src: idx_v0_ret, consts: consts1,
        };
        ...
        let distinct: std::collections::HashSet<i64> = rec.io_examples.iter().map(|(_, o)| *o).collect();
        assert!(distinct.len() >= 2, "should have diverse outputs");
        let _ = prog3;
        println!("synthetic_record test passed ✓ ({} distinct outputs)", distinct.len());
    }

    /// Verify warm-start synthesis: hand-crafted description for n+1, should solve in ≤ 50 steps.
    #[test]
    fn warm_start_synthesis_test() {
        use crate::benchmark::{Problem, Example, Value};
        // Build a simple n+1 problem
        let examples: Vec<Example> = (1i64..=8).map(|n| Example {
        ...
        slots[0] = SlotDesc { op: 0, s1: 0, s2: idx_const1, gate_cmp: 2, gate_lhs: 0, gate_rhs: 0, else_val: 0 };
        ...
        let desc = UniversalProgramDescription {
            ...
            cond_cmp: 5, cond_lhs: 0, cond_rhs: 0,
            ret_src: idx_v0,
            consts,
        };
