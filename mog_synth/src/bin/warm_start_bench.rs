...
use mog_synth::synthesis::{
    synthesize_universal_warm_start, UniversalProgramDescription,
};
...
fn main() {
    ...
    let n_args_filter: Option<usize> = arg_value(&args, "--n-args")
        .and_then(|s| s.parse().ok());
    ...
    if has_flag(&args, "--predict-py") {
        let model = arg_value(&args, "--model")
            .unwrap_or_else(|| "models/metalearner_1arg.pt".to_string());
        run_predict_mode(&model, warm_steps, n_args_filter, warm_only);
        return;
    }

    // ── Mode: load descriptions from JSONL file ────────────────────────────────
    let desc_path = arg_value(&args, "--descriptions")
        .unwrap_or_else(|| { eprintln!("Usage: warm_start_bench --descriptions FILE [--warm-steps N]"); std::process::exit(1); });

    let file = std::fs::File::open(&desc_path)
        .unwrap_or_else(|e| { eprintln!("Cannot open {desc_path}: {e}"); std::process::exit(1); });
    ...
    eprintln!("Benchmarking {} warm-start descriptions (warm_steps={warm_steps})...", records.len());
    let mut total_warm = 0usize;
    let mut warm_wins = 0usize;
    let mut total_cold_steps = 0usize;
    ...
    for rec in &records {
        let problem = match all_problems.iter().find(|p| p.name == rec.problem_name) {
            Some(p) => p,
            None => {
                eprintln!("  [SKIP] problem '{}' not found in benchmark", rec.problem_name);
                continue;
            }
        };
        total_warm += 1;
    ...
    let warm_rate = if total_warm > 0 { warm_wins * 100 / total_warm } else { 0 };
    eprintln!("\nSummary:");
    eprintln!("  Problems tested:  {total_warm}");
    eprintln!("  Warm-start wins:  {warm_wins} ({warm_rate}%)");
    eprintln!("  Avg warm steps:   {}", if warm_wins > 0 { total_warm_steps / warm_wins } else { 0 });
}

/// Run Python meta-learner (single batch call) on all benchmark problems, then test warm-start.
fn run_predict_mode(model_path: &str, warm_steps: usize, n_args_filter: Option<usize>, warm_only: bool) {
    ...
    {
        let f = std::fs::File::create(batch_in)
            .unwrap_or_else(|e| { eprintln!("Cannot create {batch_in}: {e}"); std::process::exit(1); });
    ...
    let py_status = std::process::Command::new("python3")
        .args(["scripts/train_metalearner.py",
               "--infer", model_path,
               "--batch-in", batch_in,
               "--batch-out", batch_out])
        .status();
    ...
    #[derive(serde::Deserialize)]
    struct PredRec { name: String, description: UniversalProgramDescription }
    let desc_file = std::fs::File::open(batch_out)
        .unwrap_or_else(|e| { eprintln!("Cannot open {batch_out}: {e}"); std::process::exit(1); });
    ...
    eprintln!("  Got {} predictions. Running synthesis...\n", desc_map.len());
    ...
    for (i, problem) in problems.iter().enumerate() {
        ...
        match synthesize_universal_warm_start(problem, desc, warm_steps, cold_restarts) {
            Some((result, steps, warm_ok)) => {
                ...
                if warm_ok { warm_wins += 1; }
                eprintln!("  [{}/{}] {} → {tag} {steps} steps — {}", i+1, total, problem.name, result.method);
            }
            None => {
    ...
    let pct = if total_tested > 0 { warm_wins as f64 / total_tested as f64 * 100.0 } else { 0.0 };
...
fn build_io_rows(problem: &mog_synth::benchmark::Problem, n_args: usize) -> Option<Vec<Vec<i64>>> {
    ...
    for ex in &problem.examples {
        if ex.inputs.len() != n_args { return None; }
        let mut row: Vec<i64> = ex.inputs.iter().filter_map(|v| {
            if let Value::Int(i) = v { Some(*i) } else { None }
        }).collect();
        if row.len() != n_args { return None; }
