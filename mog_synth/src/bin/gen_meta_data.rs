...
use mog_synth::synthesis::{
    rand_description, synthetic_record, synthesize_universal_and_collect,
    MetaRecord, SoftUniversalProgram, UniversalProgramDescription,
};
...
fn known_1arg_descriptions() -> Vec<(String, UniversalProgramDescription)> {
    use mog_synth::synthesis::{SlotDesc, N_UNIV_SLOTS as NS, N_LOOP_SLOTS as NLS};
    let consts = [0f32, 1.0, -1.0, 2.0, -2.0, 10.0];
    // Pool indices
    const A: usize = 0;  const C0: usize = 1; const C1: usize = 2; const CN1: usize = 3;
    const C2: usize = 4; const CN2: usize = 5; const C10: usize = 6;
    const V0: usize = 7; const V1: usize = 8; const V2: usize = 9;
    const S0: usize = 10; const S1: usize = 11; const S2: usize = 12; const S3: usize = 13;
    const P0: usize = 16; const P1: usize = 17;
    // LIP indices
    const LA: usize = 0; const LC0: usize = 1; const LC1: usize = 2; const LCN1: usize = 3;
    const LC2: usize = 4; const LV0: usize = 7; const LV1: usize = 8; const LV2: usize = 9;

    // Default slot: identity (op=5=id, gate trivially false → else=a)
    let id_slot = SlotDesc { op: 5, s1: A, s2: A, gate_cmp: 5, gate_lhs: A, gate_rhs: A, else_val: A };
    let no_loop_init = vec![LC0; NLS]; // all loop inits = const 0
    // No-loop condition: a != a = false
    let no_loop = (5usize, A, A); // (cond_cmp, cond_lhs, cond_rhs)

    let mut result = vec![];

    let mk = |name: &str, slots: Vec<SlotDesc>, loop_init: Vec<usize>,
               cond_cmp: usize, cond_lhs: usize, cond_rhs: usize,
               ret_src: usize| -> (String, UniversalProgramDescription) {
        (name.to_string(), UniversalProgramDescription {
            n_args: 1, slots, loop_init, cond_cmp, cond_lhs, cond_rhs, ret_src, consts,
        })
    };
    ...
    // f(a) = a + 1
    {
        ...
        s[0] = SlotDesc { op: 0, s1: A, s2: C1, gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: A };
        result.push(mk("add_one", s, no_loop_init.clone(), 5, A, A, V0));
    }
    // f(a) = a + 2
    {
        let mut s = vec![id_slot.clone(); NS];
        s[0] = SlotDesc { op: 0, s1: A, s2: C2, gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: A };
        result.push(mk("add_two_val", s, no_loop_init.clone(), 5, A, A, V0));
    }
    // f(a) = a - 1
    {
        let mut s = vec![id_slot.clone(); NS];
        s[0] = SlotDesc { op: 1, s1: A, s2: C1, gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: A };
        result.push(mk("sub_one", s, no_loop_init.clone(), 5, A, A, V0));
    }
    // f(a) = a * 2
    {
        let mut s = vec![id_slot.clone(); NS];
        s[0] = SlotDesc { op: 2, s1: A, s2: C2, gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: A };
        result.push(mk("double", s, no_loop_init.clone(), 5, A, A, V0));
    }
    // f(a) = a * a
    {
        let mut s = vec![id_slot.clone(); NS];
        s[0] = SlotDesc { op: 2, s1: A, s2: A, gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: A };
        result.push(mk("square", s, no_loop_init.clone(), 5, A, A, V0));
    }
    // f(a) = a * 10
    {
        let mut s = vec![id_slot.clone(); NS];
        s[0] = SlotDesc { op: 2, s1: A, s2: C10, gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: A };
        result.push(mk("times_ten", s, no_loop_init.clone(), 5, A, A, V0));
    }
    // f(a) = a % 2 (is_even raw)
    {
        let mut s = vec![id_slot.clone(); NS];
        s[0] = SlotDesc { op: 4, s1: A, s2: C2, gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: A };
    ...
    // f(a) = if a >= 0 then a else -a  (abs)
    {
        let mut s = vec![id_slot.clone(); NS];
        // v0 = -a = 0 - a
        s[0] = SlotDesc { op: 1, s1: C0, s2: A, gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: A };
        // v1 = if a >= 0 then a else v0 (-a)   (cmp index 3 = ">=")
        s[1] = SlotDesc { op: 5, s1: A, s2: A, gate_cmp: 3, gate_lhs: A, gate_rhs: C0, else_val: V0 };
    ...
    // f(a) = if a > 0 then 1 else if a < 0 then -1 else 0  (sign)
    {
        ...
        s[0] = SlotDesc { op: 5, s1: C1, s2: C1, gate_cmp: 4, gate_lhs: A, gate_rhs: C0, else_val: CN1 };
        // v1 = if a == 0 then 0 else v0
        s[1] = SlotDesc { op: 5, s1: A, s2: A, gate_cmp: 2, gate_lhs: A, gate_rhs: C0, else_val: V0 };
        result.push(mk("sign_fn", s, no_loop_init.clone(), 5, A, A, V1));
    }
    // f(a) = a * a + a
    {
        let mut s = vec![id_slot.clone(); NS];
        s[0] = SlotDesc { op: 2, s1: A, s2: A, gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: A }; // a*a
        s[1] = SlotDesc { op: 0, s1: V0, s2: A, gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: A }; // a*a+a
    ...
    // ─────────────── Sum to N: s0=0, loop while s1!=0: s0+=s1, s1-=1 ──────────
    {
        ...
        s[3] = SlotDesc { op: 0, s1: S0, s2: S1, gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: S0 }; // s0+=s1
        s[4] = SlotDesc { op: 1, s1: S1, s2: C1, gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: S1 }; // s1-=1
        // loop_init: s0=0, s1=a, others=0
        let mut li = vec![LC0; NLS]; li[0] = LC0; li[1] = LA;
        result.push(mk("sum_to_n", s, li, 5, S1, C0, S0)); // while s1 != 0
    }
    // ─────────────── Factorial: s0=1, s1=n, while s1 != 1: s0*=s1, s1-=1 ─────
    {
        let mut s = vec![id_slot.clone(); NS];
        s[3] = SlotDesc { op: 2, s1: S0, s2: S1, gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: S0 }; // s0*=s1
        s[4] = SlotDesc { op: 1, s1: S1, s2: C1, gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: S1 }; // s1-=1
        let mut li = vec![LC0; NLS]; li[0] = LC1; li[1] = LA; // s0=1, s1=a
    ...
    // ─────────────── Fibonacci: s0=0,s1=1, while s2!=n: tmp=s1, s1=s0+s1, s0=tmp, s2+=1 ─
    {
        ...
        s[3] = SlotDesc { op: 0, s1: S0, s2: S1, gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: S1 }; // new fib = s0+s1
        s[4] = SlotDesc { op: 5, s1: S1, s2: S1, gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: S1 }; // keep old s1
        s[5] = SlotDesc { op: 0, s1: S2, s2: C1, gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: S2 }; // s2+=1
        ...
        s2[3] = SlotDesc { op: 1, s1: S0, s2: C1, gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: S0 }; // s0-=1
        s2[4] = SlotDesc { op: 0, s1: S1, s2: S2, gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: S1 }; // new=s1+s2
        s2[5] = SlotDesc { op: 5, s1: S2, s2: S2, gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: S2 }; // keep s2
        let mut li2 = vec![LC0; NLS]; li2[0] = LA; li2[1] = LC0; li2[2] = LC1; // s0=a, s1=0, s2=1
        result.push(mk("fibonacci", s2, li2, 5, S0, C0, S1)); // while s0 != 0, return s1
    }
    // ─────────────── Count digits: s0=n, s1=count, while s0 != 0: s0/=10, s1+=1 ─
    {
        let mut s = vec![id_slot.clone(); NS];
        s[3] = SlotDesc { op: 3, s1: S0, s2: C10, gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: S0 }; // s0/=10
        s[4] = SlotDesc { op: 0, s1: S1, s2: C1, gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: S1 }; // s1+=1
        let mut li = vec![LC0; NLS]; li[0] = LA; li[1] = LC0; // s0=a, s1=0
    ...
    {
        let mut s = vec![id_slot.clone(); NS];
        s[0] = SlotDesc { op: 0, s1: A, s2: C1, gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: A }; // v0 = a+1
        s[1] = SlotDesc { op: 2, s1: A, s2: V0, gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: A }; // v1 = a*(a+1)
        s[2] = SlotDesc { op: 3, s1: V1, s2: C2, gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: A }; // v2 = v1/2
    ...
    // ─────────────── celsius to fahrenheit: 9*a/5 + 32 ──────────────────────
    {
        ...
        s[0] = SlotDesc { op: 2, s1: A, s2: C2, gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: A }; // 2*a
    ...
    {
        let mut s = vec![id_slot.clone(); NS];
        s[3] = SlotDesc { op: 2, s1: S0, s2: C2, gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: S0 }; // s0*=2
        s[4] = SlotDesc { op: 1, s1: S1, s2: C1, gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: S1 }; // s1-=1
        let mut li = vec![LC0; NLS]; li[0] = LC1; li[1] = LA; // s0=1, s1=n
    ...
    // Loop: s0+=s2, s1/=10, s2=s1%10 (sequential: s2 reads updated s1)
    {
        let mut s = vec![id_slot.clone(); NS];
        // Init slot 0: v0 = a % 10
        s[0] = SlotDesc { op: 4, s1: A, s2: C10, gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: A };
        // Loop slot 3 (s0): s0 = s0 + s2  (uses digit from prev iter)
        s[3] = SlotDesc { op: 0, s1: S0, s2: S2, gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: S0 };
        // Loop slot 4 (s1): s1 = s1 / 10
        s[4] = SlotDesc { op: 3, s1: S1, s2: C10, gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: S1 };
        // Loop slot 5 (s2): s2 = s1 % 10  (reads UPDATED s1 → next digit)
        s[5] = SlotDesc { op: 4, s1: S1, s2: C10, gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: S2 };
        let mut li = vec![LC0; NLS]; li[0] = LC0; li[1] = LA; li[2] = LV0; // s0=0, s1=a, s2=v0
    ...
    // Loop: s0/=10, s1=s3+s2, s2=s0%10, s3=s1*10
    {
        ...
        s[3] = SlotDesc { op: 3, s1: S0, s2: C10, gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: S0 };
        // Loop slot 4 (s1): s1 = s3 + s2  (staged_prev + digit_prev)
        s[4] = SlotDesc { op: 0, s1: S3, s2: S2, gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: S1 };
        // Loop slot 5 (s2): s2 = s0 % 10  (reads UPDATED s0)
        s[5] = SlotDesc { op: 4, s1: S0, s2: C10, gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: S2 };
        // Loop slot 6 (s3): s3 = s1 * 10  (reads UPDATED s1)
        s[6] = SlotDesc { op: 2, s1: S1, s2: C10, gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: S3 };
        let mut li = vec![LC0; NLS]; li[0] = LA; li[1] = LC0; li[2] = LV0; li[3] = LC0; // s0=a, s1=0, s2=v0, s3=0
    ...
    // Loop: s0-=1, s1+=s2, s2=s0*s0 (reads UPDATED s0)
    {
        ...
        s[3] = SlotDesc { op: 1, s1: S0, s2: C1, gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: S0 };
        // Loop slot 4 (s1): s1 = s1 + s2  (uses square from prev iter / init)
        s[4] = SlotDesc { op: 0, s1: S1, s2: S2, gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: S1 };
        // Loop slot 5 (s2): s2 = s0 * s0  (reads UPDATED s0 = current-1)
        s[5] = SlotDesc { op: 2, s1: S0, s2: S0, gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: S2 };
        let mut li = vec![LC0; NLS]; li[0] = LA; li[1] = LC0; li[2] = LV0; // s0=n, s1=0, s2=a^2
    ...
    {
        let mut s = vec![id_slot.clone(); NS];
        s[3] = SlotDesc { op: 2, s1: S0, s2: S1, gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: S0 };
        s[4] = SlotDesc { op: 1, s1: S1, s2: C1, gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: S1 };
        let mut li = vec![LC0; NLS]; li[0] = LC1; li[1] = LA; // s0=1, s1=n
    ...
    // s0=0, s1=n, s2=1 (sign), while s1!=0: temp=s1*s2 stored in s3, s0+=s3, s2*=-1, s1-=1
    {
        ...
        s[3] = SlotDesc { op: 0, s1: S0, s2: S3, gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: S0 };
        // slot 4 (s1): s1 = s1 - 1
        s[4] = SlotDesc { op: 1, s1: S1, s2: C1, gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: S1 };
        // slot 5 (s2): s2 = s2 * -1  (flip sign)
        s[5] = SlotDesc { op: 2, s1: S2, s2: CN1, gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: S2 };
        // slot 6 (s3): s3 = s1 * s2  (compute NEXT term using updated s1 and s2)
        s[6] = SlotDesc { op: 2, s1: S1, s2: S2, gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: S3 };
        ...
        s[0] = SlotDesc { op: 5, s1: A, s2: A, gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: A }; // v0=a (identity)
        let mut li = vec![LC0; NLS]; li[0] = LC0; li[1] = LA; li[2] = LC1; li[3] = LV0;
    ...
    // Uses conditional gate: slot 3 (s0) conditionally updates
    {
        ...
        s[0] = SlotDesc { op: 4, s1: A, s2: C2, gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: A }; // v0=a%2
        // v1 = a * 2 (will use for 3*s0 computation)
        s[1] = SlotDesc { op: 0, s1: A, s2: A, gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: A }; // v1=a+a=2a
    ...
    // v0=a%2, v1=if v0==0 then 1 else 0
    {
        let mut s = vec![id_slot.clone(); NS];
        s[0] = SlotDesc { op: 4, s1: A, s2: C2, gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: A }; // v0=a%2
        s[1] = SlotDesc { op: 5, s1: C1, s2: C1, gate_cmp: 2/*==*/, gate_lhs: V0, gate_rhs: C0, else_val: C0 }; // v1=v0==0?1:0
    ...
    {
        let mut s = vec![id_slot.clone(); NS];
        s[0] = SlotDesc { op: 2, s1: A, s2: A, gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: A }; // v0=a*a
        s[1] = SlotDesc { op: 2, s1: V0, s2: A, gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: A }; // v1=v0*a
    ...
    {
        let mut s = vec![id_slot.clone(); NS];
        s[0] = SlotDesc { op: 2, s1: A, s2: CN1, gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: A }; // v0=-a
    ...
    // ─────────────── clamp_positive: max(a, 0) ────────────────────────────────
    {
        ...
        s[0] = SlotDesc { op: 5, s1: A, s2: A, gate_cmp: 3/*>=*/, gate_lhs: A, gate_rhs: C0, else_val: C0 };
    ...
    {
        let mut s = vec![id_slot.clone(); NS];
        s[0] = SlotDesc { op: 3, s1: A, s2: C2, gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: A }; // a/2
    ...
    {
        const S4: usize = 14; const S5: usize = 15;
        const P0: usize = 16; const _P1: usize = 17;
        let _ = (S4, S5); // used in loop_init positions only
        let mut s = vec![id_slot.clone(); NS];
        // Loop slot 3 (s0): s0 = s0 / 10
        s[3] = SlotDesc { op: 3, s1: S0, s2: C10, gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: S0 };
        // Loop slot 4 (s1): s1 = s1 + 1
        s[4] = SlotDesc { op: 0, s1: S1, s2: C1, gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: S1 };
        // Post slot 9 (p0): if s1==0 then 1 else s1
        s[9] = SlotDesc { op: 5, s1: C1, s2: C1, gate_cmp: 2/*==*/, gate_lhs: S1, gate_rhs: C0, else_val: S1 };
        let mut li = vec![LC0; NLS]; li[0] = LA; li[1] = LC0; // s0=n, s1=0
    ...
    // Slot 6: s3=s1_new (staging = new a for next iter) — reads UPDATED s1
    {
        ...
        s[4] = SlotDesc { op: 5, s1: S2, s2: S2, gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: S1 };
        // Slot 5 (s2): s2 = s3 + s2 (reads OLD s3 and OLD s2; s3 from prev iter = old a)
        s[5] = SlotDesc { op: 0, s1: S3, s2: S2, gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: S2 };
        // Slot 6 (s3): s3 = s1 (reads NEW s1 = old_b, for next iteration)
        s[6] = SlotDesc { op: 5, s1: S1, s2: S1, gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: S3 };
        let mut li = vec![LC0; NLS]; li[0] = LA; li[1] = LC0; li[2] = LC1; li[3] = LC0;
    ...
    // Same as fibonacci_v2 but init: s1=2, s2=1, s3=2
    {
        ...
        s[5] = SlotDesc { op: 0, s1: S3, s2: S2, gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: S2 };
        s[6] = SlotDesc { op: 5, s1: S1, s2: S1, gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: S3 };
        let mut li = vec![LC0; NLS]; li[0] = LA; li[1] = LC2; li[2] = LC1; li[3] = LC2;
    ...
    {
        let mut s = vec![id_slot.clone(); NS];
        s[0] = SlotDesc { op: 4, s1: A, s2: C10, gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: A }; // v0=a%10
        s[3] = SlotDesc { op: 2, s1: S0, s2: S2, gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: S0 }; // s0*=s2
        s[4] = SlotDesc { op: 3, s1: S1, s2: C10, gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: S1 }; // s1/=10
        s[5] = SlotDesc { op: 4, s1: S1, s2: C10, gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: S2 }; // s2=s1%10
        let mut li = vec![LC0; NLS]; li[0] = LC1; li[1] = LA; li[2] = LV0;
    ...
    // Slot 3: s0/=10, slot 4: s1=max(s1,s2) via gate, slot 5: s2=s0%10(updated)
    {
        ...
        s[4] = SlotDesc { op: 5, s1: S2, s2: S2, gate_cmp: 4/*>*/, gate_lhs: S2, gate_rhs: S1, else_val: S1 };
        s[5] = SlotDesc { op: 4, s1: S0, s2: C10, gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: S2 }; // s2=s0%10
        let mut li = vec![LC0; NLS]; li[0] = LA; li[1] = LC0; li[2] = LV0;
    ...
    // s0=n, while s0>=10: s0/=10, return s0
    {
        let mut s = vec![id_slot.clone(); NS];
        s[3] = SlotDesc { op: 3, s1: S0, s2: C10, gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: S0 }; // s0/=10
        let mut li = vec![LC0; NLS]; li[0] = LA;
    ...
    // Like digit_sum but divide by 2 instead of 10
    {
        let mut s = vec![id_slot.clone(); NS];
        s[0] = SlotDesc { op: 4, s1: A, s2: C2, gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: A }; // v0=a%2
        s[3] = SlotDesc { op: 0, s1: S0, s2: S2, gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: S0 }; // s0+=s2
        s[4] = SlotDesc { op: 3, s1: S1, s2: C2, gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: S1 };  // s1/=2
        s[5] = SlotDesc { op: 4, s1: S1, s2: C2, gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: S2 };  // s2=s1%2
        let mut li = vec![LC0; NLS]; li[0] = LC0; li[1] = LA; li[2] = LV0;
    ...
    // v0=x+1, v1=v0*x=(x+1)*x=x^2+x, v2=v1*2=2x^2+2x, p0=v2+v0=2x^2+3x+1
    {
        let mut s = vec![id_slot.clone(); NS];
        s[0] = SlotDesc { op: 0, s1: A, s2: C1, gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: A };   // v0=a+1
        s[1] = SlotDesc { op: 2, s1: V0, s2: A,  gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: V0 }; // v1=v0*a=(x+1)*x
        s[2] = SlotDesc { op: 2, s1: V1, s2: C2, gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: V1 }; // v2=v1*2=2x^2+2x
        s[9] = SlotDesc { op: 0, s1: V2, s2: V0, gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: V2 }; // p0=v2+v0=2x^2+3x+1
        result.push(mk("polynomial_v2", s, no_loop_init.clone(), 5/* a!=a */, A, A, P0)); // no loop
    }
    ...
    //   S3=parity=S5%2 (s[6]), S4=steps+1 (s[7]), S5=new_n=if parity==0:S5/2 else S2 (s[8])
    {
        const S4: usize = 14; const S5: usize = 15;
        let mut s = vec![id_slot.clone(); NS];
        s[3] = SlotDesc { op: 2, s1: S5, s2: C2,  gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: S0 }; // s0=s5*2=2n (s5 old)
        s[4] = SlotDesc { op: 0, s1: S0, s2: S5,  gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: S1 }; // s1=s0+s5=3n (s0 new, s5 old)
        s[5] = SlotDesc { op: 0, s1: S1, s2: C1,  gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: S2 }; // s2=s1+1=3n+1
        s[6] = SlotDesc { op: 4, s1: S5, s2: C2,  gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: S3 }; // s3=s5%2 (parity, s5 old)
        s[7] = SlotDesc { op: 0, s1: S4, s2: C1,  gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: S4 }; // s4=steps+1
        // s5 = if s3==0 (even) then s5/2 else s2 (3n+1); s5 still old here
        s[8] = SlotDesc { op: 3, s1: S5, s2: C2,  gate_cmp: 2, gate_lhs: S3, gate_rhs: C0, else_val: S2 }; // s5=n/2 or 3n+1
        let mut li = vec![LC0; NLS]; li[5] = LA; // s5=a(n), s4=0(steps), rest=0
    ...
    // Post: p0 = if a==0 then 1 else s2
    {
        const S3: usize = 13;
        let mut s = vec![id_slot.clone(); NS];
        s[3] = SlotDesc { op: 4, s1: S3, s2: C10, gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: S0 }; // s0=s3%10 (digit)
        s[4] = SlotDesc { op: 4, s1: S0, s2: C2,  gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: S1 }; // s1=s0%2 (parity; s0=new digit)
        s[5] = SlotDesc { op: 0, s1: S2, s2: C1,  gate_cmp: 2, gate_lhs: S1, gate_rhs: C0, else_val: S2 }; // s2+=1 if parity==0
        s[6] = SlotDesc { op: 3, s1: S3, s2: C10, gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: S3 }; // s3/=10 (old s3)
        s[9] = SlotDesc { op: 5, s1: C1, s2: C1,  gate_cmp: 2, gate_lhs: A, gate_rhs: C0, else_val: S2 }; // p0=1 if a==0 else s2
        let mut li = vec![LC0; NLS]; li[3] = LA; // s0=0,s1=0,s2=0(count),s3=a(counter)
        result.push(mk("count_even_digits_v2", s, li, 4/* > */, S3, C0, P0)); // while s3>0, return p0
    }
    ...
    // S0=digit, S1=parity, S2=sum, S3=n_counter
    {
        ...
        let mut s = vec![id_slot.clone(); NS];
        s[3] = SlotDesc { op: 4, s1: S3, s2: C10, gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: S0 }; // s0=s3%10 (digit)
        s[4] = SlotDesc { op: 4, s1: S0, s2: C2,  gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: S1 }; // s1=s0%2 (parity)
        s[5] = SlotDesc { op: 0, s1: S2, s2: S0,  gate_cmp: 2, gate_lhs: S1, gate_rhs: C1, else_val: S2 }; // s2+=digit if parity==1
        s[6] = SlotDesc { op: 3, s1: S3, s2: C10, gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: S3 }; // s3/=10
        let mut li = vec![LC0; NLS]; li[3] = LA; // s2=0(sum), s3=a(counter)
    ...
    {
        let mut s = vec![id_slot.clone(); NS];
        s[3] = SlotDesc { op: 0, s1: S0, s2: C1,  gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: S0 }; // s0+=1 (k++)
        s[4] = SlotDesc { op: 0, s1: S1, s2: S0,  gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: S1 }; // s1+=s0 (T+=new_k)
        // Post: p0 = if s1==a then 1 else 0
        s[9] = SlotDesc { op: 5, s1: C1, s2: C1,  gate_cmp: 2, gate_lhs: S1, gate_rhs: A, else_val: C0 };
        result.push(mk("triangular_check_v2", s, no_loop_init.clone(), 0/* < */, S1, A, P0)); // while s1<a, return p0
    }
    ...
    // Slot order 3(unused)→4(compute check)→5(update count)→6(increment i).
    {
        const S3: usize = 13;
        let mut s = vec![id_slot.clone(); NS];
        s[4] = SlotDesc { op: 4, s1: A,  s2: S3,  gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: S1 }; // s1=A%s3(=A%i)
        s[5] = SlotDesc { op: 0, s1: S2, s2: C1,  gate_cmp: 2, gate_lhs: S1, gate_rhs: C0, else_val: S2 }; // s2+=1 if s1==0
        s[6] = SlotDesc { op: 0, s1: S3, s2: C1,  gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: S3 }; // s3+=1 (i++)
        let mut li = vec![LC0; NLS]; li[3] = LC1; // s2=0(count), s3=1(i)
    ...
    // Same structure: S1=A%i, S2=sum, S3=i; add S3 (old i) when S1==0.
    {
        ...
        let mut s = vec![id_slot.clone(); NS];
        s[4] = SlotDesc { op: 4, s1: A,  s2: S3,  gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: S1 }; // s1=A%s3(=A%i)
        s[5] = SlotDesc { op: 0, s1: S2, s2: S3,  gate_cmp: 2, gate_lhs: S1, gate_rhs: C0, else_val: S2 }; // s2+=s3(=i) if s1==0
        s[6] = SlotDesc { op: 0, s1: S3, s2: C1,  gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: S3 }; // s3+=1 (i++)
        let mut li = vec![LC0; NLS]; li[3] = LC1; // s2=0(sum), s3=1(i)
    ...
    {
        let mut s = vec![id_slot.clone(); NS];
        s[0] = SlotDesc { op: 2, s1: C10, s2: C10, gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: C10 }; // v0=100
        s[1] = SlotDesc { op: 2, s1: V0,  s2: C10, gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: V0  }; // v1=1000
        s[3] = SlotDesc { op: 0, s1: S0, s2: S1,  gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: S0 }; // s0+=prev_term
        s[4] = SlotDesc { op: 3, s1: V1, s2: S2,  gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: S1 }; // s1=1000/i (old i)
        s[5] = SlotDesc { op: 0, s1: S2, s2: C1,  gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: S2 }; // s2=i+1
        s[9] = SlotDesc { op: 0, s1: S0, s2: S1,  gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: S0 }; // p0=s0+last_term
        let mut li = vec![LC0; NLS]; li[2] = LC1; // s0=0,s1=0,s2=1(i starts at 1)
    ...
    {
        let mut s = vec![id_slot.clone(); NS];
        s[3] = SlotDesc { op: 0, s1: S0, s2: C1,  gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: S0 }; // s0+=1 (guess++)
        s[4] = SlotDesc { op: 2, s1: S0, s2: S0,  gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: S1 }; // s1=s0^2 (new s0)
        // Post: p0 = if s1==a then 1 else 0
        s[9] = SlotDesc { op: 5, s1: C1, s2: C1,  gate_cmp: 2, gate_lhs: S1, gate_rhs: A, else_val: C0 };
        result.push(mk("is_perfect_square_v2", s, no_loop_init.clone(), 0/* < */, S1, A, P0)); // while s1<a, return p0
    }
    ...
    // s0=1 (p); while s0<n: s0*=2; return s0
    {
        let mut s = vec![id_slot.clone(); NS];
        s[3] = SlotDesc { op: 2, s1: S0, s2: C2,  gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: S0 }; // s0*=2
        let mut li = vec![LC0; NLS]; li[0] = LC1; // s0=1
    ...
    {
        let mut s = vec![id_slot.clone(); NS];
        s[0] = SlotDesc { op: 5, s1: CN1, s2: CN1, gate_cmp: 0/*<*/, gate_lhs: A, gate_rhs: C0, else_val: C0 }; // v0=a<0?-1:0
        s[1] = SlotDesc { op: 5, s1: C1,  s2: C1,  gate_cmp: 4/*>*/, gate_lhs: A, gate_rhs: C0, else_val: V0 };  // v1=a>0?1:v0
    ...
    {
        let mut s = vec![id_slot.clone(); NS];
        s[0] = SlotDesc { op: 5, s1: C1, s2: C1, gate_cmp: 3/*>=*/, gate_lhs: A, gate_rhs: C0, else_val: CN1 }; // v0=a>=0?1:-1
        s[1] = SlotDesc { op: 5, s1: C0, s2: C0, gate_cmp: 2/*==*/, gate_lhs: A, gate_rhs: C0, else_val: V0  }; // v1=a==0?0:v0
    ...
    // s0=acc=0, s1=counter=1, while s1 <= a: s0+=s1, s1+=1. Return s0.
    {
        ...
        s[3] = SlotDesc { op: 0, s1: S0, s2: S1, gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: S0 }; // s0+=s1
        s[4] = SlotDesc { op: 0, s1: S1, s2: C1, gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: S1 }; // s1+=1
        let mut li = vec![LC0; NLS]; li[0] = LC0; li[1] = LC1; // s0=0, s1=1
    ...
    {
        let mut s = vec![id_slot.clone(); NS];
        s[3] = SlotDesc { op: 1, s1: S0, s2: C1,  gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: S0 }; // s0-=1
        s[4] = SlotDesc { op: 0, s1: S1, s2: S0,  gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: S1 }; // s1+=new_s0
        let mut li = vec![LC0; NLS]; li[0] = LA; li[1] = LC0; // s0=a, s1=0
    ...
    // Cleanest: s0=acc, s1=counter=a; while s1!=0: s0+=s1, s1-=1. Same as sum_to_n.
    {
        ...
        s[3] = SlotDesc { op: 0, s1: S0, s2: S1, gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: S0 }; // s0+=s1
        s[4] = SlotDesc { op: 1, s1: S1, s2: C1, gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: S1 }; // s1-=1
        let mut li = vec![LC0; NLS]; li[0] = LC0; li[1] = LA; // s0=0, s1=a
    ...
    {
        let mut s = vec![id_slot.clone(); NS];
        s[3] = SlotDesc { op: 0, s1: S1, s2: S0,  gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: S1 }; // dummy — need temp
        // Actually forward: need s2=s0*s0 first, then s1+=s2, then s0+=1
        // Reuse slot 3 for square, slot 4 for acc, slot 5 for counter
        s[3] = SlotDesc { op: 2, s1: S0, s2: S0, gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: S2 };  // s2=s0^2 wait no s2 is written later
        // s0=counter, s1=acc, s2=staging: slot3 computes s0^2 into s2, slot4 adds to s1, slot5 increments s0
        s[3] = SlotDesc { op: 2, s1: S0, s2: S0, gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: S0 }; // s0=s0*s0 (overwrite counter!)
        ...
        s[3] = SlotDesc { op: 1, s1: S0, s2: C1,  gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: S0 }; // s0-=1
        s[4] = SlotDesc { op: 0, s1: S1, s2: S2,  gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: S1 }; // s1+=s2
        s[5] = SlotDesc { op: 2, s1: S0, s2: S0,  gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: S2 }; // s2=s0^2
        // init v0=a*a, then s0=a, s1=0, s2=v0=a^2
        s[0] = SlotDesc { op: 2, s1: A, s2: A, gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: A }; // v0=a^2
        let mut li = vec![LC0; NLS]; li[0] = LA; li[1] = LC0; li[2] = LV0; // s0=a,s1=0,s2=a^2
    ...
    // slot5: s2=s3+s2 (OLD s3 = prev cur, OLD s2 = prev); slot6: s3=NEW s1
    {
        ...
        s[5] = SlotDesc { op: 0, s1: S3, s2: S2,  gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: S2 };
        s[6] = SlotDesc { op: 5, s1: S1, s2: S1,  gate_cmp: 2, gate_lhs: A, gate_rhs: A, else_val: S3 };
        let mut li = vec![LC0; NLS]; li[0] = LA; li[1] = LC1; li[2] = LC2; li[3] = LC1;
    ...
    if has_flag(&args, "--synthetic") {
        ...
        while kept < n {
            ...
            if let Some(record) = synthetic_record(&desc, n_eval, seed ^ 0xdeadbeef) {
                ...
                kept += 1;
                if kept % 100 == 0 {
                    eprintln!("  {kept}/{n} kept ({tried} tried, {:.0}% accept rate)",
        ...
        let filter_n_args: Option<usize> = arg_value(&args, "--n-args")
            .and_then(|s| s.parse().ok());
        let max_steps: usize = arg_value(&args, "--max-steps")
            .and_then(|s| s.parse().ok())
            .unwrap_or(400);
        let all_problems = get_benchmark(1);
        let problems: Vec<_> = all_problems.into_iter().filter(|p| {
            let n = p.examples.first().map(|ex| ex.inputs.len()).unwrap_or(0);
            // Only integer problems; filter by n_args if specified
            p.examples.iter().all(|ex| ex.inputs.iter().all(|v| matches!(v, mog_synth::benchmark::Value::Int(_))))
        ...
        for (i, problem) in problems.iter().enumerate() {
            ...
            match synthesize_universal_and_collect(problem, max_steps) {
                Some((result, params)) => {
                    let n_args = problem.examples.first()
                        .map(|ex| ex.inputs.len()).unwrap_or(1);
                    let prog = SoftUniversalProgram::new_from_params(n_args, params);
                    let desc = prog.params_to_description();
                    let io_examples: Vec<(Vec<i64>, i64)> = problem.examples.iter()
                        .map(|ex| {
                            let inputs: Vec<i64> = ex.inputs.iter().filter_map(|v| {
                                if let mog_synth::benchmark::Value::Int(i) = v { Some(*i) } else { None }
                            }).collect();
                            (inputs, ex.expected)
                        }).collect();
        ...
        eprintln!("Generating {} reps × {} known descriptions...", reps, known_descs.len());
        ...
        for (name, desc) in &known_descs {
            ...
            while kept < reps {
                ...
                if let Some(rec) = synthetic_record(desc, n_eval, seed) {
                    ...
                    let line = serde_json::to_string(&named)
                        .unwrap_or_else(|e| panic!("json error: {e}"));
                    writeln!(w, "{line}").unwrap();
                    kept += 1;
                    total_kept += 1;
                }
                if tried > reps * 100 { break; }
            }
            eprintln!("  {name} → {kept}/{reps} generated");
        }
        eprintln!("Done: {total_kept} total from {} known program types.", known_descs.len());
        ...
        let in_path = arg_value(&args, "--augment")
            .unwrap_or_else(|| { eprintln!("--augment requires a JSONL file path"); std::process::exit(1); });
        let reps: usize = arg_value(&args, "--reps")
            .and_then(|s| s.parse().ok())
            .unwrap_or(200);
        let n_eval: usize = arg_value(&args, "--n-eval")
        ...
        let input_file = std::fs::File::open(&in_path)
            .unwrap_or_else(|e| { eprintln!("cannot open {in_path}: {e}"); std::process::exit(1); });
        ...
        eprintln!("Augmenting {} benchmark descriptions × {reps} reps...", records.len());
        let mut total_kept = 0usize;
        ...
        for rec in &records {
            ...
            while kept < reps {
                ...
                if tried > reps * 50 { break; } // give up if acceptance too low
            }
            eprintln!("  {} → {kept}/{reps} augmented", rec.fn_name);
        }
        eprintln!("Done: {total_kept} total augmented pairs from {} descriptions.", records.len());
        ...
        let bench1: Vec<_> = bench_problems.iter().filter(|p| {
            let n = p.examples.first().map(|ex| ex.inputs.len()).unwrap_or(0);
            n == 1 && p.examples.iter().all(|ex| ex.inputs.iter().all(|v| matches!(v, Value::Int(_))))
        }).collect();

        eprintln!("Checking {} known descriptions against {} 1-arg benchmark problems...",
            known_descs.len(), bench1.len());
        ...
        for (kname, desc) in &known_descs {
            ...
            for problem in &bench1 {
                ...
                if all_correct { matched_problems.push(problem); }
            }

            if !matched_problems.is_empty() {
                eprintln!("  {kname} → matches {} problem(s): {}",
                    matched_problems.len(),
                    matched_problems.iter().map(|p| p.name.as_str()).collect::<Vec<_>>().join(", "));
                total_matches += matched_problems.len();

                for problem in &matched_problems {
                    // Record 1: use benchmark's actual I/O examples
                    let bench_io: Vec<(Vec<i64>, i64)> = problem.examples.iter()
                        .map(|ex| {
                            let inputs: Vec<i64> = ex.inputs.iter().filter_map(|v| {
                                if let Value::Int(i) = v { Some(*i) } else { None }
                            }).collect();
                    ...
                    let mut tried = 0usize;
                    while kept < reps {
                        let seed: u64 = (total_records as u64 * 7919) ^ (tried as u64 * 1009) ^ 0xbeef;
                        tried += 1;
                        if let Some(aug) = synthetic_record(desc, n_eval, seed) {
        ...
        let matched_names: std::collections::HashSet<String> = {
            ...
            for (_kname, desc) in &known_descs2 {
                ...
                for problem in &bench1 {
                    ...
                    if all_correct { names.insert(problem.name.clone()); }
                }
            }
            names
        };
        let unmatched: Vec<_> = bench1.iter().filter(|p| !matched_names.contains(&p.name)).collect();
        if !unmatched.is_empty() {
            eprintln!("  Unmatched ({}):", unmatched.len());
            for p in &unmatched { eprintln!("    - {}", p.name); }
        }
        eprintln!("Done: {total_matches} description→problem matches, {total_records} training records.");

    } else {
        eprintln!("Usage:");
        eprintln!("  gen_meta_data --synthetic N [--n-args A] [--n-eval K] [--seed S] [--out FILE]");
