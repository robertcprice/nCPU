...
Inputs (JSON from stdin OR --examples flag):
...
from soft_synth import (
    ...
    N_CMPS, N_OPS_EXT, _pool, _sps, _lip,
)
...
def run(req: dict, model_path: str, n_steps: int = 400, fast: bool = True) -> dict:
    ...
    if check_discrete(params, examples, n_args):
        code = description_to_code(desc, name)
        return {"solved": True, "code": code, "method": "py_warm_exact", "description": desc}

    # Step 2: Perturbation search (1-field corrections, ~120ms)
    corrected = perturb_search(params, examples, n_args)
    if corrected is not None:
        code = description_to_code(desc, name)
        return {"solved": True, "code": code, "method": "py_warm_perturb", "description": desc}
    ...
    if result.get("solved"):
        code = description_to_code(desc, name)
        return {"solved": True, "code": code, "method": result.get("method", "py_warm_grad"), "description": desc}

    # Step 4: Cold restarts (very slow, last resort)
    result = synthesize(examples, n_args, n_steps=n_steps, n_restarts=3)
    if result.get("solved"):
        code = description_to_code(desc, name)
        return {"solved": True, "code": code, "method": "py_cold", "description": desc}
