...
def synthesize(examples, n_args: int = 1, n_steps: int = 800,
    ...
    if init_params is not None:
        ...
        with torch.no_grad():
            if check_discrete(ip, examples, n_args):
                return {"solved": True, "steps": 0, "loss": 0.0, "method": "warm_exact"}

        # 2. Perturbation search — fix near-miss predictions (1 field wrong)
        with torch.no_grad():
            corrected = perturb_search(ip, examples, n_args)
        if corrected is not None:
            return {"solved": True, "steps": 0, "loss": 0.0, "method": "warm_perturb"}
        ...
        if solved:
            return {"solved": True, "steps": steps, "loss": 0.0, "method": "warm_grad"}
    ...
    for _ in range(n_restarts):
        ...
        if solved:
            return {"solved": True, "steps": steps, "loss": 0.0, "method": "cold"}
