...
class TestComputePrograms:
    ...
    def test_sum_1_to_10(self, kernel):
        source = (PROJECT_ROOT / "programs" / "arithmetic" / "sum_1_to_10.asm").read_text()
    ...
    def test_fibonacci_iterative(self, kernel):
        source = (PROJECT_ROOT / "programs" / "arithmetic" / "fibonacci_iterative.asm").read_text()
        kernel.load_program_from_asm(source)
        result = kernel.execute()
        assert result.stop_reason_name == "HALT"
    ...
    def test_factorial(self, kernel):
        source = (PROJECT_ROOT / "programs" / "arithmetic" / "factorial.asm").read_text()
        kernel.load_program_from_asm(source)
        result = kernel.execute()
        assert result.stop_reason_name == "HALT"
    ...
    def test_collatz(self, kernel):
        source = (PROJECT_ROOT / "programs" / "algorithms" / "collatz.asm").read_text()
        kernel.load_program_from_asm(source)
        result = kernel.execute()
        assert result.stop_reason_name == "HALT"

    def test_gcd(self, kernel):
        source = (PROJECT_ROOT / "programs" / "algorithms" / "gcd.asm").read_text()
        kernel.load_program_from_asm(source)
        result = kernel.execute()
        assert result.stop_reason_name == "HALT"
...
class TestCrossModeVerification:
    ...
    PROGRAMS = [
        "arithmetic/sum_1_to_10.asm",
        "arithmetic/fibonacci_iterative.asm",
        "arithmetic/factorial.asm",
        "algorithms/collatz.asm",
        "algorithms/gcd.asm",
        "arithmetic/max_of_two.asm",
        "arithmetic/min_of_two.asm",
        "arithmetic/absolute_value.asm",
        "bitwise/is_even_odd.asm",
        "arithmetic/triangular_number.asm",
    ]
