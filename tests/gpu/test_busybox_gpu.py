...
DEMOS_GPU_DIR = str(PROJECT_ROOT / "demos" / "gpu")
HAS_BUSYBOX = Path(BUSYBOX).exists()

# Add demos/ to path so we can import demo modules
if DEMOS_DIR not in sys.path:
    sys.path.insert(0, DEMOS_DIR)
if DEMOS_GPU_DIR not in sys.path:
    sys.path.insert(0, DEMOS_GPU_DIR)
