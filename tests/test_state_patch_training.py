"""Tests for state-patch dataset preparation and training."""
...
def _write_state_patch_trajectory(path: Path) -> None:
    events = [
        {
        ...
        },
        {
            ...
            "metadata": {
                ...
                "adaptation_descriptor": {
                    "signal_projection": [0.5, -0.25, 0.1],
...
class TestStatePatchTraining(unittest.TestCase):
    def test_build_state_patch_training_examples(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            trajectory_path = Path(tmpdir) / "trajectory.jsonl"
            _write_state_patch_trajectory(trajectory_path)
            trajectory = load_trajectory(trajectory_path)

            examples = build_state_patch_training_examples(trajectory)

        self.assertEqual(len(examples), 1)
        self.assertEqual(examples[0].update_kind, "verify_failure_descriptor")
        self.assertEqual(examples[0].feature_vector[:3], [0.5, -0.25, 0.1])
        self.assertEqual(examples[0].target_vector[:3], [0.2, 0.4, -0.3])

    def test_train_state_patch_head(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            trajectory_path = Path(tmpdir) / "trajectory.jsonl"
            _write_state_patch_trajectory(trajectory_path)
            trajectory = load_trajectory(trajectory_path)
            ...
            train_path = Path(tmpdir) / "state_patch_train.jsonl"
            val_path = Path(tmpdir) / "state_patch_val.jsonl"
            ...
            checkpoint_path = Path(tmpdir) / "state_patch_head.pt"
            metrics = train_state_patch_head(
