"""Tests for latent action head dataset preparation and training."""
...
class TestLatentActionTraining(unittest.TestCase):
    ...
    def test_build_latent_action_training_examples(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            trajectory_path = Path(tmpdir) / "trajectory.jsonl"
            self._write_trajectory(trajectory_path)
            trajectory = load_trajectory(trajectory_path)

            examples = build_latent_action_training_examples(trajectory)

        self.assertEqual([example.target_action for example in examples], ["think", "patch"])
        self.assertEqual(examples[0].allowed_actions, ["think", "write"])
        self.assertEqual(examples[1].allowed_actions, ["patch", "think", "fail"])
        self.assertGreater(len(examples[0].feature_vector), 24)

    def test_train_latent_action_head(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            trajectory_path = Path(tmpdir) / "trajectory.jsonl"
            self._write_trajectory(trajectory_path)
            trajectory = load_trajectory(trajectory_path)
            ...
            train_path = Path(tmpdir) / "latent_action_train.jsonl"
            val_path = Path(tmpdir) / "latent_action_val.jsonl"
            import json
            ...
            checkpoint_path = Path(tmpdir) / "latent_action_head.pt"
            metrics = train_latent_action_head(
