"""Tests for executable-thought evaluation reports."""
...
class TestExecutableThoughtEvaluation(unittest.TestCase):
    def test_evaluation_report_contains_metrics_and_program_summary(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            shared_model = _TinyHiddenModel(hidden_dim=6)
            shared_tokenizer = _TinyTokenizer()
            ...
            report = evaluate_executable_thought_head(
                train_path=train_path,
                val_path=val_path,
            ...
            self.assertIn("program_summary", report["splits"]["train"])
            self.assertIn("update_kind_breakdown", report["splits"]["val"])
            self.assertIn(
                "verify_failure_descriptor",
                report["splits"]["val"]["update_kind_breakdown"],
            )
            self.assertGreaterEqual(report["splits"]["train"]["program_summary"]["unique_programs"], 1)
            self.assertIn(
                "task_update_consistency",
                report["splits"]["train"]["program_summary"],
            )
