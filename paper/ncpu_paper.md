...
| `section_synthesized_software.md` | The verified-synthesis stack as a system: skill formats v1/v2/v3, mined vocabularies, three memory banks, delivery surfaces, synthesized Pong | 22-rule Pong game logic discovered from I/O example pairs alone (14 synthesized + 8 composed, zero sweep mismatches over up to 160,801 cases/rule); negative memory dedupes failed search (1,415 rejections cold, 0 new warm) | `node tools/pong_synthesis/finalize_pong_rules.mjs`; `cargo test` in `kernels/npcot_wasm`; `tests/synthesis_api/`, `tests/registry/`, `tests/mcp_server/` | `tools/pong_synthesis/pong_rules_final.json` |
