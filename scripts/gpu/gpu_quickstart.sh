...
#   3. scp -P PORT /path/to/nCPU/artifacts/archives/egdc_deploy.tar.gz root@HOST:/root/
#      scp -P PORT /path/to/nCPU/scripts/gpu/gpu_train.py root@HOST:/root/
...
if [ ! -f gpu_train.py ]; then
    echo "Missing gpu_train.py. Upload scripts/gpu/gpu_train.py to /root before running this quick start."
    exit 1
fi
