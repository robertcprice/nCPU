...
LOCK_DIR="$QUEUE_DIR/.queue_single.lock"
...
wait_for_output() {
  local output_path="$1"
  until [[ -f "$output_path" ]]; do
    log "waiting for prerequisite output: $output_path"
    sleep 60
  done
}
...
main() {
  ...
  ensure_ollama_running
  wait_for_output "/root/evalplus_humaneval_full_qwen35_4b_some_timeout300_rerun_20260310.json"
  ...
  run_bigcodebench_step \
    "bigcodebench-hard-4b" \
    "qwen3.5:4b" \
    "$QUEUE_DIR/bigcodebench_hard_instruct_qwen35_4b_some.json" \
    "$QUEUE_DIR/bigcodebench_hard_instruct_qwen35_4b_some.log"
  ...
  run_step \
    "mbpp-full-4b" \
    "$QUEUE_DIR/evalplus_mbpp_full_qwen35_4b_some.json" \
    "$QUEUE_DIR/evalplus_mbpp_full_qwen35_4b_some.log" \
    python3 -u ncpu/self_optimizing/run_evalplus_benchmark.py \
      ...
      --output "$QUEUE_DIR/evalplus_mbpp_full_qwen35_4b_some.json"
