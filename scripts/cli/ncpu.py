#!/usr/bin/env python3
"""
nCPU — Neural CPU Command Line Interface (LEGACY / DIRECT LAUNCHER).

⚠️  DEPRECATION NOTICE (April 2026)
    This direct launcher is being superseded by the unified surface:
        python -m ncpu gpu                 # THE HERO: Rust Metal GPU computer (preferred)
        python -m ncpu discover
        python -m ncpu text --interactive
        python -m ncpu lab
        ncpu-lab ...

    The Rust Metal kernel (`kernels/rust_metal/`, `ncpu_run` binary) is now the
    production path for speed, determinism, full OS, and the in-shader neural ALU.

    This script still works for many workloads but is no longer the recommended
    entry point. See docs/maintainers/REPO_REORGANIZATION_PROPOSAL.md and
    docs/architecture/LAYERING.md.

Run programs on a CPU where every ALU operation is a trained neural network.

Usage (legacy):
    python scripts/cli/ncpu.py --program programs/arithmetic/sum_1_to_10.asm
    ...
"""
...
def run_fast(args):
    ...
    if args.binary:
        ...
        print("Usage: python scripts/cli/ncpu.py --program programs/arithmetic/fibonacci.asm  (neural mode)")
        print("       python scripts/cli/ncpu.py --binary firmware.bin --fast       (GPU tensor mode)")
        return 1
    else:
...
Examples:
    ...
    python scripts/cli/ncpu.py --program programs/arithmetic/fibonacci.asm --trace

    # Run inline assembly
    python scripts/cli/ncpu.py --inline "MOV R0, 42; HALT"

    # Run ARM64 binary on GPU tensor CPU (maximum speed)
    python scripts/cli/ncpu.py --binary firmware.bin --fast

    # Run on GPU compute shader (Metal) — qemu-style, millions of IPS
    python scripts/cli/ncpu.py --program programs/arithmetic/sum_1_to_10.asm --compute

    # ARM64 binary on Metal compute shader
    python scripts/cli/ncpu.py --binary firmware.bin --compute
        """
    )
