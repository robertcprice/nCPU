"""Byte-level tokenizer for Python code generation.

MVP approach: each byte (0-255) is a token, plus 4 special tokens.
Simple but sufficient for proof-of-concept; can swap in a real BPE later.
...
class PythonCodeTokenizer:
    """Byte-level tokenizer for Python source code.

    Each character is encoded as its UTF-8 byte(s). Non-ASCII characters
    produce multiple tokens. Special tokens are appended for diffusion
    control (MASK, PAD, BOS, EOS).
    """
    ...
    def encode(self, code: str, add_bos_eos: bool = True) -> List[int]:
        """Encode a Python code string into byte-level token IDs.
    ...
    def decode(self, token_ids: List[int], skip_special: bool = True) -> str:
        """Decode token IDs back into a Python code string.
        ...
        Returns:
            Decoded Python source code string.
        """
