...
from egdc.mog.routing.direct_router import solve_problem_direct
from egdc.mog.solvers.search_solver import solve_problem as solve_with_search
...
class MogOrchestrator:
    def __init__(
        self,
        memory_root: str | Path = "egdc/mog/routing/pathway_memory",
    ...
    def solve(self, problem: MogBenchmarkProblem) -> OrchestratorResult:
        # --- Strategy 1: Differentiable program search (highest coverage) ---
        search_result = solve_with_search(problem, use_compiler=self.use_real_compiler)
        if search_result.success:
            blocked = self.check_anti_patterns(search_result.method, search_result.code)
            if not blocked:
                self._record(problem, search_result.method, search_result.code, True, None)
                return OrchestratorResult(
                    success=True, method="search", code=search_result.code,
                    family=search_result.method, interp_pass=True, compiler_pass=search_result.compiler_pass,
                )

        # --- Strategy 2: Direct structured synthesis ---
        ...
        # --- Strategy 3: Retrieval from pathway memory ---
        ...
        # --- Strategy 4: Body completion (if checkpoint available) ---
        ...
        if synth is not None:
            family = synth.template or "unknown"
            code = synth.code or ""
        elif search_result.code:
            family = search_result.method
            code = search_result.code
        self._record(problem, family, code, False, error_msg)
