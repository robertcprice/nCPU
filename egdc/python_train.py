"""Training script for Python code masked diffusion transformer.

Trains on HumanEval canonical solutions (or other code corpora).
MDLM objective: cross-entropy loss on masked positions only.

Usage:
    python -m egdc.python_train --dataset humaneval --epochs 100 --model_size tiny
    python -m egdc.python_train --dataset humaneval --epochs 50 --batch_size 8 --eval_every 10
"""
...
from egdc.python_tokenizer import PythonCodeTokenizer, MASK_TOKEN, PAD_TOKEN
...
def generate_python(
    ...
    """Generate Python code via iterative unmasking.

    No ISA slot constraints -- Python is free-form byte sequences.

    Args:
        model: trained Python diffusion model
...
def train(
    ...
    checkpoint_dir: str = "checkpoints/python",
    ...
    for epoch in range(epochs):
        ...
        # Optional: generate a sample after each eval_every epochs
        if eval_every and (epoch + 1) % eval_every == 0:
            print("\n--- Sample generation ---")
            model.eval()
            sample = generate_python(
                model, spec_tokens=None, seq_len=128, num_steps=32,
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Train Python code masked diffusion model"
    )
    parser.add_argument("--dataset", choices=["humaneval", "corpus"],
                        default="humaneval", help="Training dataset")
    ...
    parser.add_argument("--checkpoint_dir", type=str,
                        default="checkpoints/python_egdc")
    ...
    parser.add_argument("--seq_len", type=int, default=512,
                        help="Max sequence length for solutions")
    parser.add_argument("--spec_len", type=int, default=256,
                        help="Max sequence length for prompts/specs")
    parser.add_argument("--repeat", type=int, default=50,
                        help="Repeat dataset N times (164 problems is small)")
    parser.add_argument("--eval_every", type=int, default=10,
                        help="Generate sample every N epochs")
    parser.add_argument("--resume", type=str, default=None,
                        help="Path to checkpoint to resume from")
    parser.add_argument("--eval_humaneval", action="store_true",
                        help="Run HumanEval evaluation after training")
    parser.add_argument("--eval_k", type=int, default=1,
                        help="k for pass@k evaluation")
    parser.add_argument("--eval_num_problems", type=int, default=None,
                        help="Number of HumanEval problems to evaluate")
    args = parser.parse_args()

    # Model config
    if args.model_size == "tiny":
        config = PythonDiffusionConfig.tiny()
    elif args.model_size == "small":
        config = PythonDiffusionConfig.small()
    else:
        config = PythonDiffusionConfig.medium()

    # Override seq_len in config
    ...
    model = PythonMaskedDiffusion(config)
    device = get_device()
    ...
    if args.dataset == "humaneval":
        from egdc.humaneval import HumanEvalDataset
        dataset = HumanEvalDataset(
            seq_len=args.seq_len,
            spec_len=args.spec_len,
            repeat=args.repeat,
        )
        print(f"Dataset: {len(dataset)} samples ({len(dataset) // args.repeat} "
              f"problems x {args.repeat} repeats)")
    else:
    ...
    # Optional HumanEval evaluation
    if args.eval_humaneval:
        print("\n=== Running HumanEval Evaluation ===")
        from egdc.humaneval import evaluate_humaneval
        tokenizer = PythonCodeTokenizer()
        results = evaluate_humaneval(
            model=model,
            tokenizer=tokenizer,
            generate_fn=generate_python,
        ...
        # Save results
        import json
        results_path = Path(args.checkpoint_dir) / "humaneval_results.json"
        # Remove per_problem for cleaner save (it's large)
        save_results = {k: v for k, v in results.items() if k != "per_problem"}
