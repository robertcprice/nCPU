...
fn some_but_not_all_three_quantifiers_disagree() {
    ...
    let every = w.holds(&quantified(
        Quantifier::Every,
        "agent",
        quant_body("write", "report"),
    ));
    let some = w.holds(&quantified(
        Quantifier::Some,
        "agent",
        quant_body("write", "report"),
    ));
    let no = w.holds(&quantified(
        Quantifier::No,
        "agent",
        quant_body("write", "report"),
    ));
    ...
    assert_eq!(
        every,
        Some(false),
        "EVERY must be false: editor is a counterexample"
    );
    assert_eq!(
        some,
        Some(true),
        "SOME must be true: teacher writes a report"
    );
...
fn universal_with_unknown_member_is_none_not_overderived_true() {
    ...
    assert_eq!(
        some,
        Some(true),
        "existential should be witnessed-true by teacher"
    );
}
...
fn existential_false_when_all_members_determined_false() {
    ...
    assert_eq!(
        every,
        Some(false),
        "EVERY must be false: every member is a counterexample"
    );
}
...
fn universal_verdict_tracks_the_body_not_just_the_category() {
    ...
    let every_write = w.holds(&quantified(
        Quantifier::Every,
        "agent",
        quant_body("write", "report"),
    ));
    let every_read = w.holds(&quantified(
        Quantifier::Every,
        "agent",
        quant_body("read", "book"),
    ));
    ...
    assert_eq!(
        every_read,
        Some(false),
        "every agent reads a book must be FALSE (none read)"
    );
    assert_ne!(
...
fn existential_respects_the_patient_argument() {
    ...
    let some_report = w.holds(&quantified(
        Quantifier::Some,
        "agent",
        quant_body("write", "report"),
    ));
    let some_memo = w.holds(&quantified(
        Quantifier::Some,
        "agent",
        quant_body("write", "memo"),
    ));
...
fn parser_captures_force_and_evaluator_is_scope_correct() {
    ...
    assert_eq!(
        force(&every),
        Some(Quantifier::Every),
        "parsed universal force: {every:?}"
    );
    assert_eq!(
        force(&some),
        Some(Quantifier::Some),
        "parsed existential force: {some:?}"
    );
    assert_eq!(
        force(&no),
        Some(Quantifier::No),
        "parsed negative force: {no:?}"
    );
    ...
    let v_every = w.holds(&quantified(
        Quantifier::Every,
        "agent",
        quant_body("write", "report"),
    ));
    let v_some = w.holds(&quantified(
        Quantifier::Some,
        "agent",
        quant_body("write", "report"),
    ));
    let v_no = w.holds(&quantified(
        Quantifier::No,
        "agent",
        quant_body("write", "report"),
    ));
    ...
    assert_eq!(
        v_every,
        Some(false),
        "EVERY agent writes a report must be FALSE (editor counterexample)"
    );
    assert_eq!(
        v_some,
        Some(true),
        "SOME agent writes a report must be TRUE (teacher does)"
    );
    assert_eq!(
        v_no,
        Some(false),
        "NO agent writes a report must be FALSE (teacher does)"
    );
    assert!(
...
fn parser_world_build_then_quantified_query_tracks_body() {
    ...
    assert_eq!(
        v_writes,
        Some(true),
        "every teacher writes a report -> TRUE"
    );
    assert_eq!(
        v_reads,
        Some(false),
        "every teacher reads a book -> FALSE (teacher does not)"
    );
    assert_ne!(
        v_writes, v_reads,
        "universal verdict failed to track the body through the parser"
    );
}
...
fn qa_layer_quantified_answers_are_scope_correct() {
    ...
    assert!(
        every_write.starts_with("yes"),
        "every teacher writes a report -> Yes, got: {every_write}"
    );
    assert!(
        every_read.starts_with("no"),
        "every teacher reads a book -> No, got: {every_read}"
    );
    assert!(
        no_read.starts_with("yes"),
        "no teacher reads a book -> Yes, got: {no_read}"
    );
    assert!(
        some_read.starts_with("no"),
        "some teacher reads a book -> No, got: {some_read}"
    );
    ...
    assert_ne!(
        every_read, no_read,
        "every vs no over same body must differ"
    );
}
