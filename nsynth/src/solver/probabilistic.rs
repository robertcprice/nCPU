...
fn suggests_uncertainty(examples: &[Example]) -> bool {
    ...
    let float_outputs: Vec<f64> = examples.iter()
    ...
    let variance: f64 = float_outputs.iter()
        .map(|&x| (x - mean).powi(2))
        .sum::<f64>() / float_outputs.len() as f64;
...
fn suggests_sampling_process(examples: &[Example]) -> bool {
    ...
    let empty_inputs = examples.iter()
        .filter(|ex| ex.inputs.is_empty())
        .count();
...
fn infer_float_distribution(examples: &[Example]) -> DistributionType {
    let float_values: Vec<f64> = examples.iter()
        .filter_map(|ex| match &ex.expected {
            Value::Float(b) => Some(f64::from_bits(*b)),
            _ => None,
        })
...
fn infer_int_distribution(examples: &[Example], _first: i64) -> DistributionType {
    let int_values: Vec<i64> = examples.iter()
        .filter_map(|ex| match &ex.expected {
            Value::Int(i) => Some(*i),
            _ => None,
        })
    ...
    let variance: f64 = int_values.iter()
...
pub fn solve_probabilistic_problem(problem: &Problem) -> SolveResult {
    ...
    // Generate code
    let code = crate::prob::codegen::generate_probabilistic_code(
        &dist,
        &params,
...
fn create_distribution(
    ...
    match dist_type {
        ...
        DistributionType::Categorical => {
            let values: Vec<i64> = examples.iter()
                .filter_map(|ex| match &ex.expected {
                    Value::Int(i) => Some(*i),
                    _ => None,
                })
        ...
        DistributionType::Poisson => {
            ...
            Ok(ProbDistribution::Poisson { lambda: lambda.max(0.1) })
        }
        DistributionType::Uniform => {
            let values: Vec<f64> = examples.iter()
                .filter_map(|ex| match &ex.expected {
            ...
            Ok(ProbDistribution::Uniform { low: min, high: max })
        }
        DistributionType::Unknown => {
            Err("Unknown distribution type".to_string())
        }
    }
}
