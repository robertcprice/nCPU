...
pub(super) fn try_combinator(problem: &Problem, name: &str) -> Option<SolveResult> {
    // REGISTRY PRIMITIVES must synthesize from first principles — the library
    // hole-power atoms below re-open the alias hole the op_library registry-op
    // gate closed: on 2-4 all-positive example rows, `array_sum` resolved to
    // max_subarray_sum(xs) and `length` to count_positives(xs) — wrong programs
    // (and foreign helper fn names) leaking into every downstream component.
    // Same principle, same category key; the MBPP cascade is untouched.
...
fn fold(l: &[i64], op: &str) -> Option<i64> {
    match op {
        // CHECKED folds: fresh-holdout arrays sampled by problem_from_reference
        // can be large; an unchecked native sum/product PANICS with overflow in
        // debug builds (observed: composition pipeline aborted mid-verify). An
        // overflowing fold cannot match any finite expected value -> None.
        "sum" => l.iter().try_fold(0i64, |a, &x| a.checked_add(x)),
        "product" => l.iter().try_fold(1i64, |a, &x| a.checked_mul(x)),
