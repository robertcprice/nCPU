...
fn search_only_generalizes_on_array_holdout_cases() {
    assert_search_generalizes(
        "array_max_v0",
        vec![
            (vec![Value::int_array(&[-3, -9, -1])], -1),
            (vec![Value::int_array(&[10, 2, 10])], 10),
        ],
    );
    assert_search_generalizes(
        "closure_map_sum_v0",
    ...
    assert_search_generalizes(
        ...
        vec![
            ...
            (vec![Value::int_array(&[0, 0, 1])], 1),
        ],
    );
    assert_search_generalizes(
        "array_max_elem_v0",
        vec![
            (vec![Value::int_array(&[-1, -5, -3])], -1),
            (vec![Value::int_array(&[10, 2, 10])], 10),
        ],
    );
    assert_search_generalizes(
        "max_pair_diff_v0",
        vec![
            (vec![Value::int_array(&[1, 10, 3, 20])], 17),
            (vec![Value::int_array(&[5, 5, 5])], 0),
        ],
    );
    assert_search_generalizes(
        "sum_negatives_v0",
        vec![
            (vec![Value::int_array(&[-5, 2, -1, 0])], -6),
    ...
    assert_search_generalizes(
        "interactive_sum_v0",
        vec![
            (vec![Value::int_array(&[10, -5, 3])], 8),
            (vec![Value::int_array(&[7])], 7),
        ],
    );
}
