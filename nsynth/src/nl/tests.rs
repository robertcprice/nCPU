...
// ============================================================
// Reference-implementation intake (root-cause-3: spec front door
// beyond examples-only)
// ============================================================

#[cfg(test)]
mod reference_intake_tests {
    use crate::benchmark::Value;

    #[test]
    fn synthesize_from_reference_double_no_hand_examples() {
        // A reference-ONLY spec (no hand examples): the reference's behavior is
        // the specification.
        let problem = crate::benchmark::problem_from_reference(
            "double",
            "fn double(x: i64) -> i64",
        ...
        // The seed examples were MANUFACTURED by running the reference (not hand
        // authored): every int input maps to 2 * input.
        assert!(
            !problem.examples.is_empty(),
            "seed examples must be generated by running the reference"
        );
        let mut saw_int = false;
        for example in &problem.examples {
            if let Value::Int(input) = example.inputs[0] {
                saw_int = true;
                assert_eq!(example.expected, Value::Int(input * 2));
            }
        }
        assert!(saw_int, "expected at least one int-input seed example");

        // An equivalent candidate (x + x) verifies through the EXISTING
        // verify_problem_code_strict pipeline, which runs generated_holdouts
        // differential testing against reference_code.
        crate::runtime::verify_problem_code_strict(
            &problem,
            "fn double(x: i64) -> i64 { return x + x; }",
        )
        .expect("an equivalent candidate must verify against the reference");

        // A non-equivalent candidate is rejected — proves the oracle fires.
        assert!(
            crate::runtime::verify_problem_code_strict(
                &problem,
            ...
            .is_err(),
            "a non-equivalent candidate must be rejected"
        );
    }
}
