    ...
    fn eval(self, arr: &[i64]) -> Option<i64> {
        if arr.is_empty() {
            return match self {
                ...
                PairwiseScan::FirstDecreaseIdx => Some(-1),
                PairwiseScan::LastIncreaseIdx => Some(-1),
                _ => Some(0),
            };
        }
        if arr.len() < 2 {
            return Some(match self {
                ...
                | PairwiseScan::LastIncreaseIdx => -1,
                _ => 0,
            });
        }
        match self {
