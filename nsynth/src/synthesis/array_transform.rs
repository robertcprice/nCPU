...
//
// The capability auto-miner reflects the engine's synthesizable ARRAY order-op
// surface — the `ReorderKind` variants this synthesizer can produce as a pure
// `Vec<i64> -> Vec<i64>` transform — into capability descriptors. The op set is
// derived FROM the `ReorderKind` enum (one arm of the exhaustive match below per
// variant); each descriptor's `example_cases` are AUTO-GENERATED by RUNNING the
// op's real semantics on canonical arrays, not hand-written. Adding a variant to
// `ReorderKind` makes the exhaustive `apply` match fail to compile until it is
// classified, so the mined surface cannot drift from the enum.

/// A mined array order-op capability (`[i64] -> [i64]`, length-preserving).
pub struct MinedArrayOp {
    /// Canonical lemma (== the engine surface name).
    pub lemma: &'static str,
    /// Run the op's real semantics on a canonical array, manufacturing examples.
    pub run: fn(&[i64]) -> Vec<i64>,
}

fn reorder_apply(kind: ReorderKind, input: &[i64]) -> Vec<i64> {
    // EXHAUSTIVE over ReorderKind — a new variant forces a new arm here, so the
    // mined surface stays bound to the enum.
    match kind {
        ReorderKind::SortAsc => {
        ...
        ReorderKind::Reverse => input.iter().rev().copied().collect(),
    }
}

/// Enumerate the synthesizable array order ops by reflecting `ReorderKind`. The
/// lemma + behaviour come from the enum and its real `reorder_apply`, not a hand
/// table.
pub fn mineable_array_ops() -> Vec<MinedArrayOp> {
    fn run_sort(a: &[i64]) -> Vec<i64> {
        reorder_apply(ReorderKind::SortAsc, a)
    }
    fn run_reverse(a: &[i64]) -> Vec<i64> {
        reorder_apply(ReorderKind::Reverse, a)
    }
    vec![
        MinedArrayOp {
            lemma: "sort",
...
// ── Reflective ELEMENTWISE-MAP surface (NL-BRIDGE-3A-MINER-WIDEN) ────────────
//
// The fixed elementwise `Vec<i64> -> Vec<i64>` maps this synthesizer emits as
// NAMED candidates (negate / double / increment / abs / square — the data-free
// per-element transforms below) are reflected into capability descriptors the
// SAME way `ReorderKind` reflects the order ops. The op set is DERIVED FROM the
// `ElementMapKind` enum: ONE arm per variant in the exhaustive `element_apply`
// (reference semantics) AND in `element_map_body` (the Mog the candidate
// emitter pushes). Adding a variant makes BOTH matches fail to compile until it
// is classified, so the mined surface cannot drift from the emitted surface.
// Each descriptor's `example_cases` are AUTO-GENERATED by RUNNING `element_apply`
// on the canonical probe arrays — never hand-written. Only data-free fixed maps
// are reflected (the data-fitted affine/quadratic searches are NOT a fixed named
// surface, so they are intentionally excluded — see `element_map_candidates`).

/// A fixed, data-free elementwise integer map this synthesizer emits as a named
/// `array_transform_map_*` candidate. The variant set IS the engine's fixed
/// element-map surface — the miner and the candidate emitter both reflect it.
...
impl ElementMapKind {
    /// All fixed element maps, in a stable order. The exhaustive matches in
    /// `element_apply` / `element_map_body` keep this bound to the enum.
    const ALL: [ElementMapKind; 5] = [
        ElementMapKind::Negate,
        ElementMapKind::Double,
        ElementMapKind::Increment,
        ElementMapKind::Square,
        ElementMapKind::Abs,
    ];

    /// Canonical lemma (== engine surface name) for the capability registry.
    fn lemma(self) -> &'static str {
        match self {
            ElementMapKind::Negate => "negate_each",
            ElementMapKind::Double => "double_each",
            ElementMapKind::Increment => "increment_each",
            ElementMapKind::Square => "square_each",
            ElementMapKind::Abs => "abs_each",
        }
    }
}

/// Reference semantics for one element — EXHAUSTIVE over `ElementMapKind`, so a
/// new variant forces a new arm here. All arithmetic is overflow-checked, mirroring
/// the strict verifier; on overflow we saturate (the canonical probes never
/// overflow, so this never fires for mined examples).
fn element_apply1(kind: ElementMapKind, x: i64) -> i64 {
    match kind {
        ElementMapKind::Negate => x.checked_neg().unwrap_or(i64::MAX),
        ElementMapKind::Double => x.checked_mul(2).unwrap_or(i64::MAX),
        ElementMapKind::Increment => x.checked_add(1).unwrap_or(i64::MAX),
        ElementMapKind::Square => x.checked_mul(x).unwrap_or(i64::MAX),
        ElementMapKind::Abs => x.checked_abs().unwrap_or(i64::MAX),
    }
}

/// Apply a fixed element map across a whole array (the op's `Vec<i64> -> Vec<i64>`
/// semantics) — manufactures auto-examples by running the real reference.
fn element_apply(kind: ElementMapKind, input: &[i64]) -> Vec<i64> {
    input.iter().map(|&x| element_apply1(kind, x)).collect()
}

/// The Mog per-element push body the candidate emitter uses for a fixed map —
/// EXHAUSTIVE over `ElementMapKind`, reusing the SAME `affine_expr` rendering as
/// the data-derived affine path (so no new codegen shape is introduced) and the
/// SAME abs/square bodies the named candidates already emitted.
fn element_map_body(kind: ElementMapKind) -> String {
    match kind {
        // Affine instances rendered through the existing `affine_expr` (a*item+b).
        ElementMapKind::Negate => format!("        result.push({});\n", affine_expr(-1, 0)),
        ElementMapKind::Double => format!("        result.push({});\n", affine_expr(2, 0)),
        ElementMapKind::Increment => format!("        result.push({});\n", affine_expr(1, 1)),
        ElementMapKind::Square => "        result.push(item * item);\n".to_string(),
        ElementMapKind::Abs => {
            "        if item < 0 {\n            result.push(0 - item);\n        } else {\n            result.push(item);\n        }\n".to_string()
        }
    }
...
/// A mined elementwise-map capability (`[i64] -> [i64]`, length-preserving).
pub struct MinedElementMapOp {
    /// Canonical lemma (== the engine surface name).
    pub lemma: &'static str,
    /// Run the op's real semantics on a canonical array, manufacturing examples.
    pub run: fn(&[i64]) -> Vec<i64>,
...
/// Enumerate the synthesizable fixed elementwise maps by reflecting
/// `ElementMapKind`. Lemma + behaviour come from the enum and its real
/// `element_apply`, not a hand table.
pub fn mineable_element_map_ops() -> Vec<MinedElementMapOp> {
    fn run_negate(a: &[i64]) -> Vec<i64> { element_apply(ElementMapKind::Negate, a) }
    fn run_double(a: &[i64]) -> Vec<i64> { element_apply(ElementMapKind::Double, a) }
    fn run_increment(a: &[i64]) -> Vec<i64> { element_apply(ElementMapKind::Increment, a) }
    fn run_square(a: &[i64]) -> Vec<i64> { element_apply(ElementMapKind::Square, a) }
    fn run_abs(a: &[i64]) -> Vec<i64> { element_apply(ElementMapKind::Abs, a) }
    let run_for = |k: ElementMapKind| -> fn(&[i64]) -> Vec<i64> {
        match k {
            ElementMapKind::Negate => run_negate,
            ElementMapKind::Double => run_double,
            ElementMapKind::Increment => run_increment,
            ElementMapKind::Square => run_square,
            ElementMapKind::Abs => run_abs,
        }
    };
    ElementMapKind::ALL
        .iter()
        .map(|&k| MinedElementMapOp { lemma: k.lemma(), run: run_for(k) })
        .collect()
}

/// EMERGENCE GUARD (compile + test enforced): assert the mined elementwise-map op
/// set is EXACTLY the `ElementMapKind` enum surface, and that every variant the
/// miner emits is also EMITTED as a candidate by `element_map_candidates`. The
/// exhaustive `match` fails to compile if a NEW `ElementMapKind` variant is added
/// without a decision here; the runtime assertions confirm the mined lemmas line
/// up with the enum, proving the vocabulary is DERIVED from the engine surface,
/// not hand-typed.
#[cfg(test)]
pub(crate) fn mineable_element_map_ops_exhaustive() {
    // Sample every variant. The match is EXHAUSTIVE: a new variant breaks this
    // until it is classified, so the mined surface cannot drift from the enum.
    let mined: std::collections::HashSet<&'static str> =
        mineable_element_map_ops().into_iter().map(|m| m.lemma).collect();
    let emitted_methods: std::collections::HashSet<&'static str> =
        element_map_candidates("f").into_iter().map(|(m, _)| m).collect();
    for &k in &ElementMapKind::ALL {
        // Exhaustive over ElementMapKind — adding a variant forces a new arm.
        let (lemma, method) = match k {
            ElementMapKind::Negate => ("negate_each", "array_transform_map_negate"),
            ElementMapKind::Double => ("double_each", "array_transform_map_double"),
            ElementMapKind::Increment => ("increment_each", "array_transform_map_increment"),
            ElementMapKind::Square => ("square_each", "array_transform_map_square_fixed"),
            ElementMapKind::Abs => ("abs_each", "array_transform_map_abs_fixed"),
        };
        assert_eq!(k.lemma(), lemma, "ElementMapKind::lemma drifted for {lemma}");
        assert!(mined.contains(lemma), "enum variant {lemma} missing from mined surface");
        assert!(
            emitted_methods.contains(method),
            "enum variant {lemma} not emitted as candidate {method}"
        );
    }
    // The mined surface is EXACTLY the enum (no extra, no missing).
    assert_eq!(
        mined.len(),
        ElementMapKind::ALL.len(),
        "mined element-map surface drifted from ElementMapKind enum"
    );
}

/// Emit the fixed elementwise-map candidates (one per `ElementMapKind`) into the
/// candidate list. These are DATA-FREE named maps — verified, like every other
/// candidate, by `verify_problem_code_strict`. Reflecting them through the same
/// enum the miner uses keeps the emitted surface == the mined surface.
fn element_map_candidates(fn_name: &str) -> Vec<(&'static str, String)> {
    ElementMapKind::ALL
        .iter()
        .map(|&k| {
            let method: &'static str = match k {
                ElementMapKind::Negate => "array_transform_map_negate",
                ElementMapKind::Double => "array_transform_map_double",
                ElementMapKind::Increment => "array_transform_map_increment",
                ElementMapKind::Square => "array_transform_map_square_fixed",
                ElementMapKind::Abs => "array_transform_map_abs_fixed",
            };
            (method, map_program(fn_name, &element_map_body(k)))
        })
        .collect()
}
...
fn candidates(problem: &Problem, rows: &[(Vec<i64>, Vec<i64>)]) -> Vec<(&'static str, String)> {
    ...
    if length_preserving {
        ...
        // (NL-BRIDGE-3A) Fixed elementwise maps reflected from `ElementMapKind`
        // (negate / double / increment / square / abs). These are the DATA-FREE
        // named maps the miner reflects into capabilities; emitting them here from
        // the SAME enum keeps the engine's emitted element-map surface == the mined
        // surface. Each is gated by `verify_problem_code_strict` like every other
        // candidate, so a wrong fixed map is rejected, not blindly accepted.
        out.extend(element_map_candidates(fn_name));
