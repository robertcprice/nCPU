...
- **MULTI-INPUT SCHEMAS (2026-07-04 ~5am)** — `2afda55`. (list, k) shapes open: H-MAP-SCALAR (affine(x,k) exact solve) + H-FILTER-SCALAR (predicate vs the ARGUMENT, must separate under every example's own k). Both end-to-end verified. Schema tier now: 12 schemas, 0 hand ops added since it landed. HumanEval re-sweep post-2-arg: 38/164 STABLE (honest: the 2-arg schemas are MBPP-shaped; HumanEval's 2-arg misses differ), 6 decompose solves stable across runs.
