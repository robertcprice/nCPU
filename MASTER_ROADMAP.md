...
**★★★ WP1 PRODUCT PATH LANDED (2026-07-10, branch `cursor/whole-software-product-path-59f9`).** Phases 2+3 are no longer bin-only demos: `handle_query` now one-shots whole-software for schema-decidable prose. Library modules `schema_component` + `whole_software`; route `QueryRoute::WholeSoftware`; schema door runs BEFORE `verified_nl_router` (so bare-NL schemas are not mis-labeled as tentative library matches); non-schema construction falls through to gated `propose_spec` after site/backend/cli/component; honest refuse updated to name the schema form + `NSYNTH_LOCAL_LLM_URL`. Bench: `scripts/whole_software_bench.sh`. **★★ WP2–WP7 PRODUCT PATH (this turn, same branch).** WP2 characterization bootstrap (`characterization.rs` + `ScaffoldKind::Characterization` in `try_scaffold` when ≥2 inline examples); WP3 gated decompose (`try_decompose_project` → `synthesize_project_with_contracts` + `write_verified_project`, `ScaffoldKind::Decompose`); WP4 property product helper (`parse_property_request` / `try_property_verify` / session `verify_property_spec`); WP5 component flywheel (`promote_schema_component` → `.nsynth/learned_components.json` / `NSYNTH_COMPONENTS`, best-effort after successful fill); WP6 schema miner (`schema_miner.rs` + `bin/mine_schemas`); WP7 session wire (char door + decompose fallback + promote + `BuildPlan` phases + honest refuse names examples/PROJECT). **★★ WP6b+NEXT WIRE (this turn, same branch).** (1) schema_miner now holes **integer literals** (`?cN`) so double/triple cluster (standalone 5/5); `mine_schemas --out` + `NSYNTH_MINED_TEMPLATES` load; `search_decompose::try_decompose` tries mined templates first (`decompose-mined-template`, const-product fill + end-to-end verify). (2) Example-bearing `handle_query` refuse → **Rust repo-agent lane** (`try_example_bearing_rust_lane` / `write_characterization_from_bench`) — the measured 13%→58% path is now on the product door, not script-only. (3) First-class dual-lane `bin/solve.rs` + `local_llm::propose_python_fn` (Lane A engine / Lane B gated Python union); scripts `dual_lane_solve.py` / `mbpp_repo_agent.py` use relative agent paths. **★★ JOINT-SEARCH PERF (this turn).** Multi-hole fill caches `cargo test` scores by fill fingerprint (`mh_score_cached`) across plain descent + joint combos; fixed `first_guess` commit so it keys off `best_body == default` (was dead: choice left on last candidate). **★★ PHASE-4 HARVEST + CHAR PARSE (this turn).** Successful whole-software / example-rust-lane fills append to `NSYNTH_HARVEST` JSONL (`schema_miner::append_harvest_row`) and promote components; characterization parser is paren-aware + accepts `prompt: 2->4, 3->6` via the verified_nl splitter. **★★ C1 FILTER-WORDS + CASE (this turn).** `string_synth` word shapes gain `FilterEvenLen` / `FilterOddLen` / `UpperEachWord` / `LowerEachWord`. **★★ CHAR DEFAULT-BODY FIX (this turn).** characterization scaffolds emit type-correct defaults (`0`/`false`/…) so the crate COMPILES and the hole-filler has a failing-test gradient (empty `{}` was a silent compile-floor miss). **★★ FLYWHEEL CLOSE (this turn).** (1) `MinedTemplate.normalized` stores hole-body (not cluster key) + `cluster_key` field — harvest→mine→instantiate works; (2) harvest strips to entry-fn body; (3) promote overlays live `resolve_components`/`route_component_build` same-process; (4) rust-lane failure returns `None` (honest refuse); (5) `ScaffoldedCrate.component_name` + unified `infer_fn_name`; (6) UTBUS accepts any `[i64]`→`i64` param name. **★★ UTBUS Phase A reduce expand (this turn).** `ArrayProgram` gains `Reduce::{Sum,Max,Min,Count}` (eval/cost/label/emit); enumerator stacks reduce as a 5th layer; example-match filters on **scalar** `eval_scalar` and tries **all** matches cheapest-first (so Sum vs Count agreeing on examples can still pass holdouts). Tests: array_max/min, count_positives, pure eval_scalar + enumerate coverage. Offline: `scripts/offline_smoke.sh` utbus_reduce harness. **★★ AUTO MINE-AFTER-HARVEST (this turn).** `append_harvest_row` → `maybe_refresh_templates_after_harvest` re-clusters the harvest (≥2 rows) into `NSYNTH_MINED_TEMPLATES` / `.nsynth/mined_templates.json` (disable `NSYNTH_AUTO_MINE=0`; throttle `NSYNTH_AUTO_MINE_EVERY`). Closes harvest→mine→instantiate without a manual `mine_schemas` step. **★★ UTBUS (arr,k)→i64 threshold preds (this turn).** `ElemPred::{GtK,LtK,EqK,NeK,GeK,LeK}` + signature arity 0|1; eval/emit take optional `k`; enumerator includes k-preds only when the signature declares the scalar. Native A2 `count_greater_than` / `count_eq` family. **★★ MINED-TEMPLATE STRICT ACCEPT (this turn).** `try_instantiate_templates` requires examples + declared holdouts + `verify_problem_code_strict` (not examples-only). **★★ RUST-LANE MOG DISTILL (partial, this turn).** After a successful example-Rust lane, `maybe_distill_rust_lane_to_mog` re-solves the same examples via `solve_problem` and `record_proposed_op` when `NSYNTH_LEARNED_OPS_PATH` is set (disable `NSYNTH_RUST_LANE_DISTILL=0`). Captures the re-synthesizable subset; Rust-only novel algorithms remain harvest/promote-only until a Rust→Mog path exists. **★★ UTBUS Product reduce (this turn).** `Reduce::Product` (empty→1) joins Sum/Max/Min/Count. **★★ UTBUS dual-accum + pairwise (this turn).** Closed `DualAccum::{Range,SecondMax}` + `PairwiseScan::{MaxAbsDiff,CountAdjacentDiff}` (native A3/A4 parity slice), tried before the filter→map→reduce enum. **★★ STRING WORD-COUNT TRIM + typed-enum filter (this turn).** `nonempty_split_count` + Mog loop skip-empty; typed_enum gains `filterw_even`/`filterw_odd` on word lists. **★★ UTBUS pairwise expand (this turn).** `CountIncreases` + `StrictlyIncreasing` (i64 0/1) join MaxAbsDiff/CountAdjacentDiff. **★★ UTBUS stock-profit + plateau + is_sorted (this turn).** `DualAccum::StockProfit`, `PairwiseScan::{LongestPlateau,NonDecreasing}` — closes remaining native A3/A4 closed-teacher shapes. **★★ SHARED FLYWHEEL + RUST LEARNED STORE (this turn).** `post_success_flywheel` unifies promote/harvest/distill for whole-software + Rust lane; `NSYNTH_RUST_LEARNED` JSONL + recall via `apply_rust_learned_to_lib` before hole-fill. **★★ UTBUS PrefixMaxSum + Zero pred (this turn).** `DualAccum::PrefixMaxSum`, `ElemPred::Zero` (count_zeros). **★★ EXAMPLE-FP RUST RECALL + KADANE (this turn).** `examples_fp` on Rust learned rows; `find_rust_learned` prefers fingerprint then name. UTBUS gains `PrefixMinSum`, `MaxSubarraySum` (Kadane), `SumAbsDiff`. **★★ UTBUS IndexScan + typed-enum len filters (this turn).** `SumEvenIndices`/`SumOddIndices`/`CountPeaks`; typed_enum `filterw_gt1/gt2/eq1`. **★★ UTBUS CountDistinct (this turn).** O(n²) first-occurrence scan in IndexScan. **★★ UTBUS Phase A closed-family expand (this turn).** DualAccum `SecondMin`/`MinSubarraySum`/`Median`/`GcdAll`/`LcmAll`/`MeanTrunc`/`SumSquares`/`AbsSum`/`MaxAbs`/`MinPositive`/`CountNegatives`/`CountEvens`/`CountOdds`/`SumPositives`/`SumNegatives`/`Len`/`IsEmpty`/`AllEqual`/`AnyPositive`/`AnyNegative`; Pairwise `CountDecreases`/`StrictlyDecreasing`/`NonIncreasing`/`LongestIncreasingRun`/`LongestDecreasingRun`/`CountAdjacentEq`; IndexScan `CountValleys`/`ArgMax`/`ArgMin`/`First`/`Last`/`Mode`/`ProductEvenIndices`/`ProductOddIndices`; new `KClosed` for `(arr,k)→i64`: `KthSmallest`/`KthLargest`/`FirstIndexOf`/`LastIndexOf`/`KthFromEnd` (tried before filter→map→reduce when arity has k). Reduce gains `Xor`/`BitOr`/`BitAnd`. Gate modes: `NSYNTH_UTBUS=1` (full) or `=closed` (closed families only); **product bins `coding_agent`/`solve` default to `closed` when unset**. C1 word shapes: `FilterLenGt2`/`FilterLenLt2`/`DedupAdjacent`/`SwapFirstLast`/`DropFirst`/`DropLast`/`FirstWord`/`LastWord`/`DuplicateEach` + typed-enum `filterw_lt2`/`eq2`. **★★ UTBUS/C1 Phase A expand (this turn).** DualAccum `AnyZero`/`AnyNonZero`/`AllPositive`/`AllNegative`/`AllNonPositive`/`CountZeros`/`HasDuplicate`/`MaxNegative`/`SumEvenValues`/`SumOddValues`/`AllNonNegative`/`CountNonZeros`/`AlternatingSum`/`ProductPositives`/`ProductNegatives`/`ProductEvens`/`ProductOdds`/`MeanAbsTrunc`/`FirstPositive`/`LastPositive`/`FirstNegative`/`LastNegative`/`MaxPositive`/`MinNegative`/`SumCubes`/`CountGtMean`/`CountLtMean`/`IsPalindrome`/`DotIndex`/`SumSqDiffMean` + empty-array eval fix; Pairwise `MaxIncrease`/`MaxDecrease`/`MinIncrease`/`MinDecrease`/`MeanAbsDiffTrunc`/`CountSignChanges`/`LongestNonDecreasingRun`/`LongestNonIncreasingRun`/`SumIncreases`/`SumDecreases`/`CountPlateaus`/`IsZigZag`; IndexScan `Middle`/`Second`/`SecondLast`/`MaxEvenIndices`/`MinEvenIndices`/`MaxOddIndices`/`MinOddIndices`/`ArgMaxAbs`/`ArgMinAbs`/`SumAbsEvenIndices`/`SumAbsOddIndices`/`CountEvenIndices`/`CountOddIndices`; KClosed `ElementAt`/`CountEqK`/`SumGtK`/`CountGtK`/`SumLtK`/`CountLtK`/`SumEqK`/`CountNeK`/`SumNeK`/`MaxLtK`/`MinLtK`/`MinGtK`/`MaxGtK`; C1 `FilterLenEq2`/`FilterLenGt3`/`FilterLenEq3`/`FilterLenLt3`/`DedupAll`/`SortByLen`/`TakeFirstTwo`/`DropFirstTwo`/`TakeLastTwo`/`DropLastTwo`/`CapLastWord`/`ReverseFirstWord` + typed-enum `filterw_gt3`/`eq3`/`lt3`; string→int `digit_count`/`upper_count`/`lower_count`/`vowel_count`/`consonant_count`/`alpha_count`/`space_count`/`longest_word_len`/`shortest_word_len`/`sum_word_lens`/`digit_sum`/`digit_product`. **★★ UTBUS/C1 Phase A expand (cont.).** DualAccum `XorAll`/`ProductNonZeros`/`OrAll`; Pairwise `SumSqDiff`; IndexScan `XorEvenIndices`/`XorOddIndices`; KClosed `CountGeK`/`CountLeK`/`SumGeK`/`SumLeK`; C1 `FilterLenEq4`/`CapFirstWord` + typed-enum `filterw_eq4`/`gt4`/`lt4`. **★★ UTBUS/C1 Phase A expand (cont. 2).** DualAccum `AndAll`; Pairwise `MeanSqDiffTrunc`; KClosed `FirstGeK`/`FirstLeK`; C1 `FilterLenGt4`/`FilterLenLt4`; string→int `punctuation_count`. **★★ UTBUS/C1 Phase A expand (cont. 3).** DualAccum `CountEqMean`; Pairwise `FirstIncreaseIdx`; KClosed `LastGeK`/`LastLeK`; C1 `ReverseLastWord`; string→int `tab_count`. **★★ UTBUS/C1 Phase A expand (cont. 4).** DualAccum `ProductAbs`; Pairwise `FirstDecreaseIdx`; IndexScan `OrEvenIndices`/`OrOddIndices`; KClosed `SumAbsGtK`; C1 `SortWordsDesc`; string→int `newline_count`. **★★ UTBUS/C1 Phase A expand (cont. 5).** DualAccum `CountNonNegatives`; Pairwise `LastIncreaseIdx`; IndexScan `AndEvenIndices`/`AndOddIndices`; KClosed `SumAbsLtK`; string→int `hex_digit_count`. **★★ UTBUS/C1 Phase A expand (cont. 6).** DualAccum `CountPositives`; Pairwise `LastDecreaseIdx`; IndexScan `ProductAbsEvenIndices`/`ProductAbsOddIndices`; KClosed `CountAbsGtK`; string→int `whitespace_count`. **★★ UTBUS/C1 Phase A expand (cont. 7).** DualAccum `MaxEvenValue`/`MaxOddValue`; IndexScan `SumSquaresEvenIndices`/`SumSquaresOddIndices`; KClosed `CountAbsLtK`; C1 `TakeFirstThree`/`DropFirstThree`; string→int `alnum_count`. **★★ UTBUS/C1 Phase A expand (cont. 8).** DualAccum `MinEvenValue`/`MinOddValue`; IndexScan `MeanEvenTrunc`/`MeanOddTrunc`; KClosed `SumAbsGeK`; C1 `TakeLastThree`/`DropLastThree`; string→int `non_alnum_count`. **★★ UTBUS/C1 Phase A expand (cont. 9).** DualAccum `AbsRange`; IndexScan `CountPositiveEvenIndices`/`CountPositiveOddIndices`; KClosed `SumAbsLeK`; C1 `MiddleWord`; string→int `underscore_count`. **★★ UTBUS/C1 Phase A expand (cont. 10).** IndexScan `CountNegativeEvenIndices`/`CountNegativeOddIndices`; KClosed `CountAbsEqK`; C1 `SecondWord`; string→int `hyphen_count`. **★★ UTBUS/C1 Phase A expand (cont. 11).** IndexScan `SumPositiveEvenIndices`/`SumPositiveOddIndices`; KClosed `FirstAbsGeK`; C1 `ThirdWord`; string→int `slash_count`. **★★ UTBUS/C1 Phase A expand (cont. 12).** IndexScan `SumNegativeEvenIndices`/`SumNegativeOddIndices`; KClosed `LastAbsGeK`; C1 `PenultimateWord`; string→int `dot_count`. **★★ UTBUS/C1 Phase A expand (cont. 13).** DualAccum `ProductNonNegatives`; IndexScan `CountZeroEvenIndices`/`CountZeroOddIndices`; KClosed `FirstAbsEqK`; C1 `ConcatWords`; string→int `comma_count`. **★★ UTBUS/C1 Phase A expand (cont. 14).** IndexScan `MaxAbsEvenIndices`/`MaxAbsOddIndices`; KClosed `LastAbsEqK`; C1 `SortByLenDesc`; string→int `colon_count`. **★★ UTBUS/C1 Phase A expand (cont. 15).** IndexScan `MinAbsEvenIndices`/`MinAbsOddIndices`; KClosed `CountAbsGeK`/`CountAbsLeK`; C1 `FourthWord`; string→int `semicolon_count`. **★★ UTBUS/C1 Phase A expand (cont. 16).** IndexScan `MeanAbsEvenTrunc`/`MeanAbsOddTrunc`; KClosed `FirstAbsLeK`/`LastAbsLeK`; C1 `FilterLenEq5` + typed-enum `filterw_eq5`; string→int `question_count`. **★★ UTBUS/C1 Phase A expand (cont. 17).** DualAccum `SumNonNegatives`; IndexScan `CountNonZeroEvenIndices`/`CountNonZeroOddIndices`; KClosed `SumAbsEqK`; C1 `FilterLenGt5` + typed-enum `filterw_gt5`; string→int `exclamation_count`. **★★ UTBUS/C1 Phase A expand (cont. 18).** DualAccum `MinAbs`; IndexScan `ProductNonZeroEvenIndices`/`ProductNonZeroOddIndices`; KClosed `FirstAbsGtK`/`LastAbsGtK`; C1 `FilterLenLt5` + typed-enum `filterw_lt5`; string→int `at_count`. **★★ UTBUS/C1 Phase A expand (cont. 19).** DualAccum `SumNonPositives`; IndexScan `SumNonZeroEvenIndices`/`SumNonZeroOddIndices`; KClosed `CountAbsNeK`; C1 `TakeFirstFour`; string→int `hash_count`. **★★ UTBUS/C1 Phase A expand (cont. 20).** DualAccum `CountNonPositives`; IndexScan `MaxNonZeroEvenIndices`/`MaxNonZeroOddIndices`; KClosed `SumAbsNeK`; C1 `DropFirstFour`; string→int `percent_count`. **★★ UTBUS/C1 Phase A expand (cont. 21).** DualAccum `ProductNonPositives`; IndexScan `MinNonZeroEvenIndices`/`MinNonZeroOddIndices`; KClosed `FirstAbsLtK`/`LastAbsLtK`; C1 `TakeLastFour`; string→int `dollar_count`. **★★ UTBUS/C1 Phase A expand (cont. 22).** IndexScan `CountEvenValueEvenIndices`/`CountEvenValueOddIndices`; KClosed `MaxAbsLtK`; C1 `DropLastFour`; string→int `ampersand_count`. **★★ UTBUS/C1 Phase A expand (cont. 23).** IndexScan `CountOddValueEvenIndices`/`CountOddValueOddIndices`; KClosed `MinAbsGtK`; C1 `FifthWord`; string→int `star_count`. **★★ UTBUS/C1 Phase A expand (cont. 24).** IndexScan `SumEvenValueEvenIndices`/`SumEvenValueOddIndices`; KClosed `MaxAbsGtK`; C1 `DuplicateFirstWord`; string→int `plus_count`. **★★ UTBUS/C1 Phase A expand (cont. 25).** IndexScan `SumOddValueEvenIndices`/`SumOddValueOddIndices`; KClosed `MinAbsLtK`; C1 `DuplicateLastWord`; string→int `equals_count`. **★★ UTBUS/C1 Phase A expand (cont. 26).** IndexScan `ProductEvenValueEvenIndices`/`ProductEvenValueOddIndices`; KClosed `FirstAbsNeK`; C1 `SwapSecondThird`; string→int `caret_count`. **★★ UTBUS/C1 Phase A expand (cont. 27).** IndexScan `ProductOddValueEvenIndices`/`ProductOddValueOddIndices`; KClosed `LastAbsNeK`; C1 `RotateLeftWords`; string→int `tilde_count`. **★★ UTBUS/C1 Phase A expand (cont. 28).** DualAccum `SumAbsEvens`/`SumAbsOdds`; KClosed `SumWhereAbsEqK`; C1 `RotateRightWords`; string→int `pipe_count`. **★★ UTBUS/C1 Phase A expand (cont. 29).** DualAccum `ProductAbsEvens`/`ProductAbsOdds`; KClosed `ProductWhereAbsEqK`; C1 `TakeMiddleTwo`; string→int `brace_count`. **★★ UTBUS/C1 Phase A expand (cont. 30).** IndexScan `SumAbsEvenValueEvenIndices`/`SumAbsEvenValueOddIndices`; KClosed `MaxWhereAbsEqK`; C1 `JoinWithHyphen`; string→int `bracket_count`. **★★ UTBUS/C1 Phase A expand (cont. 31).** IndexScan `SumAbsOddValueEvenIndices`/`SumAbsOddValueOddIndices`; KClosed `MinWhereAbsEqK`; C1 `JoinWithUnderscore`; string→int `paren_count`. **★★ UTBUS/C1 Phase A expand (cont. 32).** DualAccum `XorAbsAll`; IndexScan `OrAbsEvenIndices`/`OrAbsOddIndices`; C1 `JoinWithSlash`; string→int `quote_count`. **★★ UTBUS/C1 Phase A expand (cont. 33).** DualAccum `AndAbsAll`; IndexScan `AndAbsEvenIndices`/`AndAbsOddIndices`; C1 `JoinWithComma`; string→int `backtick_count`. **★★ UTBUS/C1 Phase A expand (cont. 34).** DualAccum `OrAbsAll`; IndexScan `XorAbsEvenIndices`/`XorAbsOddIndices`; C1 `JoinWithColon`; string→int `apostrophe_count`. **★★ UTBUS/C1 Phase A expand (cont. 35).** DualAccum `XorAbsEvens`/`XorAbsOdds`; KClosed `SumWhereAbsNeK`; C1 `JoinWithSemicolon`; string→int `less_than_count`. **★★ UTBUS/C1 Phase A expand (cont. 36).** DualAccum `AndAbsEvens`/`AndAbsOdds`; KClosed `ProductWhereAbsNeK`; C1 `JoinWithDot`; string→int `greater_than_count`. **★★ UTBUS/C1 Phase A expand (cont. 37).** DualAccum `OrAbsEvens`/`OrAbsOdds`; KClosed `MaxWhereAbsNeK`; C1 `JoinWithPlus`; string→int `backslash_count`. **★★ UTBUS/C1 Phase A expand (cont. 38).** DualAccum `SumSquaresEvens`/`SumSquaresOdds`; KClosed `MinWhereAbsNeK`; C1 `JoinWithEquals`; string→int `cr_count`. **★★ UTBUS/C1 Phase A expand (cont. 39).** DualAccum `SumCubesEvens`/`SumCubesOdds`; KClosed `SumWhereAbsGtK`; C1 `JoinWithAt`; string→int `null_count`. **★★ UTBUS/C1 Phase A expand (cont. 40).** DualAccum `ProductSquaresEvens`/`ProductSquaresOdds` (+ repaired SumCubes emit); KClosed `SumWhereAbsLtK`; C1 `JoinWithHash`; string→int `ff_count`. **★★ UTBUS/C1 Phase A expand (cont. 41).** DualAccum `MaxAbsEvens`/`MaxAbsOdds`; KClosed `ProductWhereAbsGtK`; C1 `JoinWithStar`; string→int `vt_count`. **★★ UTBUS/C1 Phase A expand (cont. 42).** DualAccum `MinAbsEvens`/`MinAbsOdds`; KClosed `ProductWhereAbsLtK`; C1 `JoinWithPercent`; string→int `bell_count`. **★★ UTBUS/C1 Phase A expand (cont. 43).** DualAccum `CountNonZeroEvens`/`CountNonZeroOdds`; KClosed `MaxWhereAbsGtK`; C1 `JoinWithAmpersand`; string→int `esc_count`. **★★ UTBUS/C1 Phase A expand (cont. 44).** DualAccum `SumNonZeroEvens`/`SumNonZeroOdds`; KClosed `MinWhereAbsGtK`; C1 `JoinWithCaret`; string→int `del_count`. **★★ UTBUS/C1 Phase A expand (cont. 45).** DualAccum `ProductNonZeroEvens`/`ProductNonZeroOdds`; KClosed `MinWhereAbsLtK`; C1 `JoinWithTilde`; string→int `soh_count`. **★★ UTBUS/C1 Phase A expand (cont. 46).** DualAccum `MeanAbsEvensTrunc`/`MeanAbsOddsTrunc`; KClosed `MaxWhereAbsGeK`; C1 `JoinWithTab`; string→int `stx_count`. **★★ UTBUS/C1 Phase A expand (cont. 47).** DualAccum `GcdAbsEvens`/`GcdAbsOdds`; KClosed `MinWhereAbsGeK`; C1 `JoinWithNewline`; string→int `etx_count`. **★★ UTBUS/C1 Phase A expand (cont. 48).** DualAccum `LcmAbsEvens`/`LcmAbsOdds`; KClosed `SumWhereAbsGeK`; C1 `JoinWithCR`; string→int `eot_count`. **★★ UTBUS/C1 Phase A expand (cont. 49).** DualAccum `ProductCubesEvens`/`ProductCubesOdds`; KClosed `ProductWhereAbsGeK`; C1 `JoinWithQuestion`; string→int `enq_count`. **★★ UTBUS/C1 Phase A expand (cont. 50).** DualAccum `SumAbsCubesEvens`/`SumAbsCubesOdds`; KClosed `ProductWhereAbsLeK`; C1 `JoinWithExclamation`; string→int `ack_count`. **★★ UTBUS/C1 Phase A expand (cont. 51).** DualAccum `ProductAbsCubesEvens`/`ProductAbsCubesOdds`; KClosed `SumWhereAbsLeK`; C1 `JoinWithBacktick`; string→int `bs_count`. **★★ UTBUS/C1 Phase A expand (cont. 52).** DualAccum `SumAbsSquaresEvens`/`SumAbsSquaresOdds`; KClosed `MaxWhereAbsLeK`; C1 `JoinWithDollar`; string→int `so_count`. **★★ UTBUS/C1 Phase A expand (cont. 53).** DualAccum `ProductAbsSquaresEvens`/`ProductAbsSquaresOdds`; KClosed `MinWhereAbsLeK`; C1 `JoinWithDoubleQuote`; string→int `si_count`. **★★ UTBUS/C1 Phase A expand (cont. 54).** DualAccum `MeanAbsSquaresEvensTrunc`/`MeanAbsSquaresOddsTrunc`; KClosed `CountWhereAbsGeK`; C1 `JoinWithSpace`; string→int `dle_count`. **★★ UTBUS/C1 Phase A expand (cont. 55).** DualAccum `CountPositiveEvens`/`CountPositiveOdds`; KClosed `CountWhereAbsLeK`; C1 `JoinWithPipe`; string→int `dc1_count`. **★★ UTBUS/C1 Phase A expand (cont. 56).** DualAccum `CountNegativeEvens`/`CountNegativeOdds`; KClosed `FirstWhereAbsGeK`; C1 `JoinWithBrace`; string→int `dc2_count`. **★★ UTBUS/C1 Phase A expand (cont. 57).** DualAccum `SumPositiveEvens`/`SumPositiveOdds`; KClosed `LastWhereAbsGeK`; C1 `JoinWithBracket`; string→int `dc3_count`. **★★ UTBUS/C1 Phase A expand (cont. 58).** DualAccum `SumNegativeEvens`/`SumNegativeOdds`; KClosed `FirstWhereAbsLeK`; C1 `JoinWithParen`; string→int `dc4_count`. **★★ UTBUS/C1 Phase A expand (cont. 59).** DualAccum `ProductPositiveEvens`/`ProductPositiveOdds`; KClosed `LastWhereAbsLeK`; C1 `JoinWithCloseBrace`; string→int `nak_count`. **★★ UTBUS/C1 Phase A expand (cont. 60).** DualAccum `ProductNegativeEvens`/`ProductNegativeOdds`; KClosed `FirstWhereAbsEqK`; C1 `JoinWithCloseBracket`; string→int `syn_count`. **★★ UTBUS/C1 Phase A expand (cont. 61).** DualAccum `MaxPositiveEvens`/`MaxPositiveOdds`; KClosed `LastWhereAbsEqK`; C1 `JoinWithCloseParen`; string→int `etb_count`. **★★ UTBUS/C1 Phase A expand (cont. 62).** DualAccum `MinPositiveEvens`/`MinPositiveOdds`; KClosed `FirstWhereAbsNeK`; C1 `JoinWithEllipsis`; string→int `can_count`. **★★ UTBUS/C1 Phase A expand (cont. 63).** DualAccum `MaxNegativeEvens`/`MaxNegativeOdds`; KClosed `LastWhereAbsNeK`; C1 `JoinWithArrow`; string→int `em_count`. **★★ UTBUS/C1 Phase A expand (cont. 64).** DualAccum `MinNegativeEvens`/`MinNegativeOdds`; KClosed `CountWhereAbsNeK`; C1 `JoinWithDoubleColon`; string→int `sub_count`. **★★ UTBUS/C1 Phase A expand (cont. 65).** DualAccum `MeanPositiveEvensTrunc`/`MeanPositiveOddsTrunc`; KClosed `FirstIndexWhereAbsGeK`; C1 `JoinWithDoubleSlash`; string→int `fs_count`. **★★ UTBUS/C1 Phase A expand (cont. 66).** DualAccum `MeanNegativeEvensTrunc`/`MeanNegativeOddsTrunc`; KClosed `LastIndexWhereAbsGeK`; C1 `JoinWithDoubleArrow`; string→int `gs_count`. **★★ UTBUS/C1 Phase A expand (cont. 67).** DualAccum `AllEvenPositive`/`AllOddPositive`; KClosed `FirstIndexWhereAbsLeK`; C1 `JoinWithSpaceship`; string→int `rs_count`. **★★ UTBUS/C1 Phase A expand (cont. 68).** DualAccum `AllEvenNegative`/`AllOddNegative`; KClosed `LastIndexWhereAbsLeK`; C1 `JoinWithHashArrow`; string→int `us_count`. **★★ UTBUS/C1 Phase A expand (cont. 69).** DualAccum `AnyEvenPositive`/`AnyOddPositive`; KClosed `FirstIndexWhereAbsEqK`; C1 `JoinWithColonArrow`; string→int `hex_count`. **★★ UTBUS/C1 Phase A expand (cont. 70).** DualAccum `AnyEvenNegative`/`AnyOddNegative`; KClosed `LastIndexWhereAbsEqK`; C1 `JoinWithBangArrow`; string→int `oct_count`. **★★ UTBUS/C1 Phase A expand (cont. 71).** DualAccum `AnyEvenNonZero`/`AnyOddNonZero`; KClosed `FirstIndexWhereAbsNeK`; C1 `JoinWithTildeArrow`; string→int `bin_count`. **★★ UTBUS/C1 Phase A expand (cont. 72).** DualAccum `AllEvenNonZero`/`AllOddNonZero`; KClosed `LastIndexWhereAbsNeK`; C1 `JoinWithStarArrow`; string→int `punct_count`. **★★ UTBUS/C1 Phase A expand (cont. 73).** DualAccum `AllEvenNonNegative`/`AllOddNonNegative`; C1 `JoinWithSlashArrow`; string→int `control_count`. **★★ UTBUS/C1 Phase A expand (cont. 74).** DualAccum `AllEvenNonPositive`/`AllOddNonPositive`; KClosed `FirstIndexWhereAbsGtK` (deduped vs `CountAbsEqK`); C1 `JoinWithPercentArrow`; string→int `graph_count`. **★★ UTBUS/C1 Phase A expand (cont. 75).** DualAccum `AnyEvenNonNegative`/`AnyOddNonNegative`; KClosed `LastIndexWhereAbsGtK`; C1 `JoinWithAmpArrow`; string→int `blank_count`. **★★ UTBUS/C1 Phase A expand (cont. 76).** DualAccum `AnyEvenNonPositive`/`AnyOddNonPositive`; KClosed `FirstIndexWhereAbsLtK`; C1 `JoinWithCaretArrow`; string→int `ascii_count`. **★★ UTBUS/C1 Phase A expand (cont. 77).** DualAccum `MaxEvenNonZero`/`MaxOddNonZero` (deduped vs `CountNonZeroEvens`); KClosed `LastIndexWhereAbsLtK`; C1 `JoinWithPipeArrow`; string→int `non_ascii_count`. **★★ UTBUS/C1 Phase A expand (cont. 78).** DualAccum `MaxEvenNonZero`/`MaxOddNonZero` finalized; KClosed `CountDivisibleByK`; C1 `JoinWithDollarArrow`; string→int `print_count`. **★★ UTBUS/C1 Phase A expand (cont. 79).** DualAccum `MinEvenNonZero`/`MinOddNonZero`; KClosed `SumDivisibleByK`; C1 `JoinWithAtArrow`; string→int `non_digit_count`. **★★ UTBUS/C1 Phase A expand (cont. 80).** DualAccum `MeanEvenNonZeroTrunc`/`MeanOddNonZeroTrunc`; KClosed `ProductDivisibleByK`; C1 `JoinWithQuestionArrow`; string→int `non_letter_count`. **★★ UTBUS/C1 Phase A expand (cont. 81).** DualAccum `XorEvenNonZero`/`XorOddNonZero`; KClosed `FirstDivisibleByK`; C1 `JoinWithHashBang`; string→int `zero_digit_count`. **★★ UTBUS/C1 Phase A expand (cont. 82).** DualAccum `OrEvenNonZero`/`OrOddNonZero`; KClosed `LastDivisibleByK`; C1 `JoinWithColonEq`; string→int `one_digit_count`. **★★ UTBUS/C1 Phase A expand (cont. 83).** DualAccum `AndEvenNonZero`/`AndOddNonZero`; KClosed `MaxDivisibleByK`; C1 `JoinWithPlusEq`; string→int `two_digit_count`. **★★ UTBUS/C1 Phase A expand (cont. 84).** DualAccum `SumAbsEvenNonZero`/`SumAbsOddNonZero`; KClosed `MinDivisibleByK`; C1 `JoinWithMinusEq`; string→int `three_digit_count`. **★★ UTBUS/C1 Phase A expand (cont. 85).** DualAccum `ProductAbsEvenNonZero`/`ProductAbsOddNonZero`; KClosed `FirstIndexDivisibleByK`; C1 `JoinWithStarEq`; string→int `four_digit_count`. **★★ UTBUS/C1 Phase A expand (cont. 86).** DualAccum `GcdAbsEvenNonZero`/`GcdAbsOddNonZero`; KClosed `LastIndexDivisibleByK`; C1 `JoinWithSlashEq`; string→int `five_digit_count`. **★★ UTBUS/C1 Phase A expand (cont. 87).** DualAccum `LcmAbsEvenNonZero`/`LcmAbsOddNonZero`; KClosed `AbsSumDivisibleByK`; C1 `JoinWithPercentEq`; string→int `six_digit_count`. **★★ UTBUS/C1 Phase A expand (cont. 88).** DualAccum `MeanAbsEvenNonZeroTrunc`/`MeanAbsOddNonZeroTrunc`; KClosed `AbsProductDivisibleByK`; C1 `JoinWithAmpEq`; string→int `seven_digit_count`. **★★ UTBUS/C1 Phase A expand (cont. 89).** DualAccum `MaxAbsEvenNonZero`/`MaxAbsOddNonZero`; KClosed `MaxAbsDivisibleByK`; C1 `JoinWithCaretEq`; string→int `eight_digit_count`. **★★ UTBUS/C1 Phase A expand (cont. 90).** DualAccum `MinAbsEvenNonZero`/`MinAbsOddNonZero`; KClosed `MinAbsDivisibleByK`; C1 `JoinWithPipeEq`; string→int `nine_digit_count`. **★★ UTBUS/C1 Phase A expand (cont. 91).** DualAccum `SumSquaresEvenNonZero`/`SumSquaresOddNonZero`; KClosed `GcdAbsDivisibleByK`; C1 `JoinWithTildeEq`; string→int `a_count`. **★★ UTBUS/C1 Phase A expand (cont. 92).** DualAccum `ProductSquaresEvenNonZero`/`ProductSquaresOddNonZero`; KClosed `LcmAbsDivisibleByK`; C1 `JoinWithDotEq`; string→int `b_count`. **★★ UTBUS/C1 Phase A expand (cont. 93).** DualAccum `SumCubesEvenNonZero`/`SumCubesOddNonZero`; KClosed `MeanAbsDivisibleByKTrunc`; C1 `JoinWithCommaEq`; string→int `c_count`. **★★ UTBUS/C1 Phase A expand (cont. 94).** DualAccum `ProductCubesEvenNonZero`/`ProductCubesOddNonZero`; KClosed `CountNonZeroDivisibleByK`; C1 `JoinWithSemiEq`; string→int `d_count`. **★★ UTBUS/C1 Phase A expand (cont. 95).** DualAccum `SumFourthPowersEvenNonZero`/`SumFourthPowersOddNonZero`; KClosed `SumNonZeroDivisibleByK`; C1 `JoinWithColonSlash`; string→int `e_count`. **★★ UTBUS/C1 Phase A expand (cont. 96).** DualAccum `ProductFourthPowersEvenNonZero`/`ProductFourthPowersOddNonZero`; KClosed `ProductNonZeroDivisibleByK`; C1 `JoinWithBangEq`; string→int `f_count`. **★★ UTBUS/C1 Phase A expand (cont. 97).** DualAccum `MeanFourthPowersEvenNonZeroTrunc`/`MeanFourthPowersOddNonZeroTrunc`; KClosed `MaxNonZeroDivisibleByK`; C1 `JoinWithQuestionEq`; string→int `g_count`. **★★ UTBUS/C1 Phase A expand (cont. 98).** DualAccum `SumFifthPowersEvenNonZero`/`SumFifthPowersOddNonZero`; KClosed `MinNonZeroDivisibleByK`; C1 `JoinWithAtEq`; string→int `h_count`. **★★ UTBUS/C1 Phase A expand (cont. 99).** DualAccum `ProductFifthPowersEvenNonZero`/`ProductFifthPowersOddNonZero`; KClosed `FirstNonZeroDivisibleByK`; C1 `JoinWithHashEq`; string→int `i_count`. **★★ UTBUS/C1 Phase A expand (cont. 100).** DualAccum `MeanFifthPowersEvenNonZeroTrunc`/`MeanFifthPowersOddNonZeroTrunc`; KClosed `LastNonZeroDivisibleByK`; C1 `JoinWithTildeSlash`; string→int `j_count`. **★★ UTBUS/C1 Phase A expand (cont. 101).** DualAccum `SumSixthPowersEvenNonZero`/`SumSixthPowersOddNonZero`; KClosed `AbsSumNonZeroDivisibleByK`; C1 `JoinWithStarSlash`; string→int `k_count`. **★★ UTBUS/C1 Phase A expand (cont. 102).** DualAccum `ProductSixthPowersEvenNonZero`/`ProductSixthPowersOddNonZero`; KClosed `AbsProductNonZeroDivisibleByK`; C1 `JoinWithPercentSlash`; string→int `l_count`. **★★ UTBUS/C1 Phase A expand (cont. 103).** DualAccum `MeanSixthPowersEvenNonZeroTrunc`/`MeanSixthPowersOddNonZeroTrunc`; KClosed `MeanNonZeroDivisibleByKTrunc`; C1 `JoinWithAmpSlash`; string→int `m_count`. **★★ UTBUS/C1 Phase A expand (cont. 104).** DualAccum `SumSeventhPowersEvenNonZero`/`SumSeventhPowersOddNonZero`; KClosed `MaxAbsNonZeroDivisibleByK`; C1 `JoinWithCaretSlash`; string→int `n_count`. **★★ UTBUS/C1 Phase A expand (cont. 105).** DualAccum `ProductSeventhPowersEvenNonZero`/`ProductSeventhPowersOddNonZero`; KClosed `MinAbsNonZeroDivisibleByK`; C1 `JoinWithPipeSlash`; string→int `o_count`. **★★ UTBUS/C1 Phase A expand (cont. 106).** DualAccum `MeanSeventhPowersEvenNonZeroTrunc`/`MeanSeventhPowersOddNonZeroTrunc`; KClosed `MeanAbsNonZeroDivisibleByKTrunc`; C1 `JoinWithDollarSlash`; string→int `p_count`. **★★ UTBUS/C1 Phase A expand (cont. 107).** DualAccum `SumEighthPowersEvenNonZero`/`SumEighthPowersOddNonZero`; KClosed `XorNonZeroDivisibleByK`; C1 `JoinWithBangSlash`; string→int `q_count`. **★★ UTBUS/C1 Phase A expand (cont. 108).** DualAccum `ProductEighthPowersEvenNonZero`/`ProductEighthPowersOddNonZero`; KClosed `OrNonZeroDivisibleByK`; C1 `JoinWithQuestionSlash`; string→int `r_count`. **★★ UTBUS/C1 Phase A expand (cont. 109).** DualAccum `MeanEighthPowersEvenNonZeroTrunc`/`MeanEighthPowersOddNonZeroTrunc`; KClosed `AndNonZeroDivisibleByK`; C1 `JoinWithAtSlash`; string→int `s_count`. **★★ UTBUS/C1 Phase A expand (cont. 110).** DualAccum `SumNinthPowersEvenNonZero`/`SumNinthPowersOddNonZero`; KClosed `CountPositiveDivisibleByK`; C1 `JoinWithHashSlash`; string→int `t_count`. **★★ UTBUS/C1 Phase A expand (cont. 111).** DualAccum `ProductNinthPowersEvenNonZero`/`ProductNinthPowersOddNonZero`; KClosed `CountNegativeDivisibleByK`; C1 `JoinWithDotSlash`; string→int `u_count`. **★★ UTBUS/C1 Phase A expand (cont. 112).** DualAccum `MeanNinthPowersEvenNonZeroTrunc`/`MeanNinthPowersOddNonZeroTrunc`; KClosed `SumPositiveDivisibleByK`; C1 `JoinWithCommaSlash`; string→int `v_count`. **★★ UTBUS/C1 Phase A expand (cont. 113).** DualAccum `SumTenthPowersEvenNonZero`/`SumTenthPowersOddNonZero`; KClosed `SumNegativeDivisibleByK`; C1 `JoinWithSemiSlash`; string→int `w_count`. **★★ UTBUS/C1 Phase A expand (cont. 114).** DualAccum `ProductTenthPowersEvenNonZero`/`ProductTenthPowersOddNonZero`; KClosed `ProductPositiveDivisibleByK`; C1 `JoinWithColonBang`; string→int `x_count`. **NEXT:** keep Phase A closed expand under `offline_smoke.sh` while lg-core missing; full `cargo test` once linguigenesis-core is present; bigger dual-lane union run; UTBUS Phase A parity gate vs 140-task bench.
