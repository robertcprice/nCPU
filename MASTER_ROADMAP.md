...
2. ⬜ STRING_SYNTH TRIM-OVERFIT: past the guard, `trim` synthesizes remove-ALL-spaces (passes the probe set because no probe has interior whitespace). Adding an interior-space probe (`"a b c"`) to `STRING_PROBES` should force real `.trim()` — BUT it did NOT (string_synth still emitted remove-all-spaces that fails `"a b c"→"a b c"`), so either string_synth isn't verifying against the full mined example set or the example isn't reaching it. NEEDS a string_synth soundness check. (Probe change reverted — it forces a cross-repo lg-core JSON commit without fixing the bug.)
3. ⬜ BRITTLE METHOD-STRING TESTS: `mined_sort` asserts `method=="array_transform_sort"` but this session's decompose tiers now win with `decompose-sort-val_asc` (still SORTS correctly). Rewrite these to assert BEHAVIOR (reproduces examples) not method labels.
