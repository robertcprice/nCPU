...
- **Phase 1 — MULTI-HOLE COORDINATION (model-free; the direct continuation; EXECUTING NOW).** A repo with several empty stubs across a struct + methods + free functions, with dependencies (filling A creates the type B needs). Fill ALL in dependency order. *Have:* single-target iteration (`synthesize_mined_for_fn` candidate loop), stub-gen, mutation repair. *Build:* hole enumeration + dependency ordering + fill-then-propagate in the supervisor. *Milestone:* struct + ≥3 empty methods + ≥5 tests → all green in one command, NO model.
