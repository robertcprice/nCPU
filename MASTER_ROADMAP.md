...
- **POWER-ARC MOVE 1 LANDED (2026-07-04): typed bottom-up enumeration** — canonical `69e34fd` (`solver/search_typed_enum.rs`). Expression pool typed {Str, StrList, Int}, depth ≤ 4, OE dedup by output vector, MAP as a second-order operator over the same transform set, Mog emission with deduped helpers, end-to-end re-verify. **HumanEval 86 anti_shuffle SOLVED — the search DISCOVERED join(map(split(s," "), sort_chars), " ") — 46/164 = 28.0%** (baseline 17.7%, +58% relative, hand-ops frozen). Tests 2/2 (off-space data refused). Next: widen operator surface (filter/concat/char-class), pool-as-hole-filler inside all 18 schemas, then MOVE 2 schema mining.
