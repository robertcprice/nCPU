...
def emit_site_synthesized_ts(results, target_dir: Path) -> Path:
    """Emit the Next.js site's ``synthesized.ts`` from the resolved results.

    This is the bridge into ``ncpu-site`` (a separate repo): the certified
    rules' transpiled TypeScript is written verbatim, alongside a ``RULES``
    metadata array the page renders. Only ``high``/``medium`` rules are exported
    in ``CALC`` (and therefore runnable on the page); a ``low``-confidence rule
    like ``api_bill`` is metadata-only with ``certified: false`` so the page can
    show it held back without ever executing it. Keeping this here makes the
    live page a reproducible build artifact of the demo, not hand-maintained."""
    target_dir.mkdir(parents=True, exist_ok=True)
    fns, calc_names, meta = [], [], []
    for english, ir, res in results:
        ts = (res.transpiled.get("typescript") or "").strip() \
            if res.status == "synthesized" else ""
        certified = res.confidence in ("high", "medium") and res.status == "synthesized"
        if certified and ts:
            fns.append(ts)
            calc_names.append(ir.entry_point)
        meta.append({
            "entry": ir.entry_point,
    ...
    header = (
        "// AUTO-GENERATED by demos/requirements_app/build_meterbill.py "
        "--emit-site — DO NOT EDIT BY HAND.\n"
        "//\n"
        "// Every function body below is the literal Mog -> TypeScript transpile of a\n"
        "// program nsynth synthesized from English-derived I/O examples; the nCPU\n"
        "// requirements pipeline then verified it on held-out examples + a reference\n"
        "// cross-check. Only CERTIFIED rules are exported in CALC and run live; the\n"
        "// uncertified api_bill rule is metadata-only (certified: false), shown held\n"
    ...
    parts.append("  holdout: string")
    parts.append("  refAgree: boolean | null")
    parts.append("  examples: RuleExample[]")
    parts.append("  mog: string")
    parts.append("  ts: string")
    parts.append("}")
    parts.append("")
    parts.append("export const RULES: Rule[] = " + json.dumps(meta, indent=2))
    out = "\n".join(parts) + "\n"
    dest = target_dir / "synthesized.ts"
    dest.write_text(out)
    print(f"wrote {dest} ({len(calc_names)} certified live fns: {', '.join(calc_names)})")
    return dest


def main() -> int:
    import sys
    emit_site_dir: Optional[Path] = None
    if "--emit-site" in sys.argv:
        i = sys.argv.index("--emit-site")
        if i + 1 < len(sys.argv):
            emit_site_dir = Path(sys.argv[i + 1]).expanduser().resolve()
    ...
    if emit_site_dir is not None:
        emit_site_synthesized_ts(results, emit_site_dir)
